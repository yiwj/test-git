  ...
  var trailer = function(idx, isTrailer) {
    try {
      ...
      if (isTrailer && $scope.trailerVideo) {
        ...
        $rootScope.prerollAd.prerollPlay(function() {
          trailerPmLog();
  ...
  $scope.moveFocusByKey = function(keyCode) {
    ...
    switch (keyCode) {
      ...
      case keyHandler.UP:
        ...
        break;
      case keyHandler.RIGHT:
        if(columnCount == 0 || rightIndex >= (columnCount - 1) || $scope.row[rowIndex].count -1  == rightIndex) {
          rect = {
            x: 0,
...
//        $rootScope.$broadcast('focus', 'drawer', keyCode, {x: 0, y: 0, width: 0, height: 0});// back button right 방향으로만 움직임
  ...
  var moveFocusFromWatch = function(keyCode) {
    ...
    switch (keyCode) {
      case keyHandler.LEFT:
        if (device.isRTL) {
          // [QEVENTSIXT-28595] RTL인경우, Watch 버튼에서 left 키 입력은 무시하도록 수정
          return;
        }
