...
app.controller('popupAppController', function($scope, $controller, $element, $rootScope, $timeout, focusManager, keyHandler, marquee, util, pmLog, eventKey, appService, device, audioGuidance) {
  ...
  $scope.audioGuidance = function (scope, target, element) {
    ...
    if (util.isAWSServer()) {
      if(popupParams &&
        ...
        popupParams.popupDesc === msgLang.alert_adult_3_6 &&
        device.isBlur){
        return;
      }
    ...
    if (element && element.querySelector('.text')) {
      ...
      if (element.classList && element.classList.contains('memory-list')) {
        if (element.classList.contains('check-on')) {
          btnName += ". ";
      ...
      if (util.isAWSServer()) {
        ...
        if (btnName && (element.classList.contains('btn-close'))) {
          btnName += '. ';
          btnName += msgLang.audio_button_button;
        }
      }
  ...
  $scope.showPopup = function(scope, param, onCloseCallback) {
    if (util.isAWSServer()) {
      ...
      if(param && param.popupTitle){
        params.text = param.popupTitle;

        if(param.popupDesc){
          params.text += '. ';
          params.text += param.popupDesc;
        }

        if(param.errorMsg){
          params.text += '. ';
          params.text += param.errorMsg;
        }
      }
    ...
    if (param.type === 'usb') { //usb 팝업 리스트일때만
      ...
      //[WOSLQEVENT-119153] 연결된 USB List가 USB Port Number 순으로 Sort 시킴
      if (viewUsbListArray && viewUsbListArray.length > 1) {
        viewUsbListArray.sort(function (a, b) {
      ...
      if (focusBeforeShowPopup && focusBeforeShowPopup.scope) {//마지막 갔던 포커스가 있는경우
        ...
        if (scope &&
          ...
          if (scopeElement === '') { //비어있는 target이면 default
            ...
            //[WEBOSDEFEC-15164]메인페이지에서 썸네일클릭 후 cp선택 시 앱 상세페이지에서 키무감 발생 버그 처리
            if($scope.type !== "watch") {
  ...
  $scope.executeAction = function() {
    ...
    if (focusObject.scope == $scope) {
      ...
      if (target == 'btn01') {
        ...
        l = usbItemArr.length;
        for (var i = 0; i < l; i++) {
          usbItemArr[i].classList.remove('check-on');
        }
        $element[0].getElementsByClassName(target)[0].classList.add('check-on');
