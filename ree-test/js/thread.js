    ...
    if (jsonObj.lgrecommendations) {
      try {
        ...
        for (i = 0; i < temp.length; i++) {
          ...
          obj.contentsTitle = temp[i].contentsTitle;
          obj.contents = [];
          obj.banners = [];
          ...
          try {
            ...
            for (j = 0; j < arr.length; j++) {
              ...
              // 아래 id값이 대체되기 때문에 pmlog용 따로 저장
              obj.bannerLink = arr[j].promotionBanner.bannerLink;
              obj.name = arr[j].promotionBanner.bannerName;
              obj.singleContents = arr[j].promotionBanner.singleContents;
          ...
          } else if (temp[i].contentsCategory == 'appsngames' || temp[i].contentsCategory == 'appsngameshot' || temp[i].contentsCategory == 'appsngamesnew') {
      ...
      if (jsonObj.lgrecommendations.contentsRollingInterval) {
        result.contentsRollingInterval = jsonObj.lgrecommendations.headRollingInterval;
      }
      // 2016.01.05 : tier1.5구분 플래그
      if (jsonObj.lgrecommendations.cpMetadataFlag) {
        result.cpMetadataFlag = jsonObj.lgrecommendations.cpMetadataFlag;
      }
      try {
        ...
        for (i = 0; i < temp.length; i++) {
          ...
          result.headList[j] = {};
          result.headList[j].category = temp[i].contentsCategory;
          result.headList[j].categoryText = temp[i].contentsCategoryText;
          result.headList[j].title = temp[i].headTitle;

          if (temp[i].contentsCategory === 'headmanual') {
            result.headList[j].image = temp[i].headImg;
            result.headList[j].id = temp[i].headLink;
            // header에 SingleContent일 때 pmLog시 headLink를 저장; 아래 category별로 저장 시 id값이 대체되기 때문에 따로 저장 함
            result.headList[j].headLink = temp[i].headLink;
            ...
            if(temp[i].contents != undefined && temp[i].singleContent === 'Y'){
              if(temp[i].contents.vodList != undefined) {
                result.headList[j].id = temp[i].contents.vodList[0].item_id;
                if (temp[i].contents.vodList[0].conts_type === 'TS') {
            ...
            result.headList[j].genreId = temp[i].genreId;
