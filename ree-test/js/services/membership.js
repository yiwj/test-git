discoveryService.service('membership', function($rootScope, LunaService2, eventKey, device) {
  ...
  self.actionAfterGetAdultStatus = function(callback) {
    ...
    var luna = {
        ...
        timeout : 10000,
        serviceCallback : function(response) {
          var callBackAdultStatus;
          if (response && response.value !== undefined) {
            callBackAdultStatus = device.auth.adultStatus;
            device.auth.adultStatus = response.value;
            if(callback) {
              callback();
            }
          } else {
            callBackAdultStatus = 'failed'; // LUNA failed (ex: timeout)
            callback(callBackAdultStatus); // WOSLQEVENT-95562 : 아래 comment 참고
          }
          // WOSLQEVENT-95562 : notification 앱에 토스트 관련 메세지가 2번 중복발생하는 현상 (호출은 위 else로 옮김)
          /*if (self.callback) {
            self.callback(callBackAdultStatus);
          }*/
        }
      };
      var lunaService = new LunaService2(luna);
