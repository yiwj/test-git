discoveryService.service('keyHandler', function($rootScope, $document, focusManager, util, $timeout, toast, device, $timeout, eventKey, pmLog) {
      ...
      if (focusEl.target == 'prev' || focusEl.target == 'next') {
        ...
        if (e.keyCode == that.ENTER && !scrollFocusChange) {
          ...
          if (!scrollFocusChange) {
            ...
            if (deltaY > 0)
              deltaY = 0;
            else if (deltaY < focusScope.$parent.scroll.maxScrollY)
              deltaY = focusScope.$parent.scroll.maxScrollY - 10;

            var passed;
            if (that.lastScrollKeyDownTime) {
              passed = new Date().getTime() - that.lastScrollKeyDownTime;
            }

            if (passed && passed < 1000) {
              // 연달아 scroll button을 클릭하는 경우
              // 1000초를 빼서, 얼마나 빠르게 클릭했는지 계산
              var offset = (1000 - passed) / 2;
              if (focusEl.target === 'next') {
                deltaY -= offset;
              } else {
                deltaY += offset;
              ...
              // console.log('keyHandler.handleKeyDown, deltaY=' + deltaY + ', offset=' + offset);
            }

            // console.log('keyHandler.handleKeyDown, deltaY=' + deltaY + ', passed=' + passed);

            that.lastScrollKeyDownTime = new Date().getTime();
  ...
  var handleKeyUp = function(e) {
    ...
    switch (e.keyCode) {
      case that.PLAY:
        if (focusManager.getCurrent().scope) {
          if(focusManager.getCurrent().scope.scopeName === 'player') {
            $rootScope.player.executeByKey(e.keyCode);
            break;
          } else if(focusManager.getCurrent().scope.scopeName === 'TVShowDetail' || focusManager.getCurrent().scope.scopeName === 'MovieShowDetail' ||
              focusManager.getCurrent().scope.scopeName === 'detailApp') { // [QEVENTSIXT-13619] play key에 의한 동작시 detail page에서는 아무것도 안하도록 함.
            break;
          }
        }
      ...
      case that.STOP:
        ...
        //[QEVENTSIXT-18557] Trailer 영상 앞 광고 재생중에서 Trailer 종료 될 수 있도록 함.
        //[QEVENTSIXT-25921]  Trailer 영상 앞 광고 재생중에서 pause 키 인가 하면 아무 동작하지 않도록 수정
        if ($rootScope.prerollAd.show && that.STOP === e.keyCode) {
