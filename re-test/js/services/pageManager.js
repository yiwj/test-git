discoveryService.service('pageManager', function(device, $rootScope, eventKey, pmLog, util) {
   ...
   * */
  this.movePageError = function(errorCode, scope, element, hotkey) {
    ...
    if (errorCode.indexOf('Apps3Depth') !== -1) {
      //[QEVENTSIXT-10139] 17년향 앱이 존재하지 않을 때 문구 변경 수정
      if (eCode === '011') {
        errorParam = {
          type: 'popup',
          popupTitle: msgLang.alert_adult_3_3,
      ...
      if(hotkey && errorParam && errorParam.popupDesc) {
        errorParam.popupDesc = msgLang.alert_2017_nofunc_1;
      }
    }

    if (device.param.page === 'myPage' && errorCode.indexOf('Apps3Depth') !== -1) {
    ...
    //[QEVENTSIXT-10139] 17년도 번역문구 변경 - 변역 문구가 나오지 않아서 추후에 수정 필요함.
    if(device.firstRun && device.isDeepLink && device.previousPage ==='' && errorCode.indexOf('Apps3Depth') !== -1) {
      var data = {};
      //문구변역이 나오지 않음. - 추후 수정 필요함
      data.errorCode = msgLang.alert_2017_nopage_1;
      //[NCVTDEFECT-3725] hotkey로 진입할 경우 보여주는 문구 변경 - 이상희 연구원 요청
      if(hotkey){
        data.errorCode = msgLang.alert_2017_nofunc_1;
      }
