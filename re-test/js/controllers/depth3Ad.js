app.directive('depth3Ad', function() {
  return {
    restrict: 'A',
    scope: {},
    replace: true,
    controller: 'depth3AdController',
    //templateUrl: './resources/html/depth3Ad.html'
    template: depth3AdTmpl
  };
});

app.controller('depth3AdController', function($scope, $controller, $element, $rootScope, $sce, $timeout, $compile, focusManager, keyHandler, marquee, adManager, device, pmLog, audioGuidance, util) {
  angular.extend(this, $controller('viewController', {$scope: $scope, $element: $element}));
  $scope.scopeName = 'depth3Ad';
  $scope.show = false;
  $scope.adFocus = false;
  ...
  $scope.showAD = function(response) {
    ...
    var subtemplate = angular.element(depth3AdSubTmpl);
    ...
    if ($scope.show) {
      ...
      switch (response[0]['type']) {
        case 'video' :
          //source tag 동적 생성
          var videoSrcUrl = response.srcUrl; //로그심기 위한 변수
          var sourcetemplate = angular.element(sourceTagTmpl);
          angular.element($element[0].getElementsByTagName('video')[0]).append($compile(sourcetemplate)($scope));
          if(!$element[0].classList.contains("item-video")) {
            $element[0].classList.add("item-video");
          ...
          document.querySelector("[depth3-ad]").style.visibility = 'visible';
...
//          if(device.q["X-Device-Platform"] === "W16R"){
          ...
          //[QEVENTSIXT-11671] 썸네일 잠시 출력되었다가 썸네일 부분 Black 으로 표시됨 (재현이 안되어 로그 심음)
          setTimeout(function () {
            pmLog.write(pmLog.LOGKEY.ST_AD_DEPTH3AD, {
              event : 'play',
              depth3ad : videoSrcUrl
            });
...
//          }
  ...
  $scope.hideAD = function() {
    ...
    if($scope.adFocus){
      //광고에 포커스 존재하고 hover-mode가 아닐때 버튼에 포커스가게 수정
      if (document.body.classList && !document.body.classList.contains('hover-mode')){
        /*동영상 광고가 닫혔을 때 포커스 앱상세로 이동하게 수정*/
          $rootScope.$broadcast('focusDefaultDetailApp');
      }
    }
    ...
    if (item) {
      ...
      if(item === 'depth3Ad') {
        ...
        if(!$scope.fullScreen) {
          ...
          //------------------------style 삭제 start--------------------------//
          if (element) {
            //console.log('depth3Ad element', element);
          ...
          //------------------------style 삭제 end--------------------------//
          //[QEVENTSIXT-13449] Focus 사라짐
        } else if(item === 'depth3Ad') {
            var currItem = 'close';
    ...
    switch (keyCode) {
      case keyHandler.LEFT:
        if ($scope.fullScreen) {
          ...
          device.isdepth3AdFocused = false;
      ...
      case keyHandler.UP:
        if ($scope.fullScreen) {
          ...
          device.isdepth3AdFocused = false;
          if(document.getElementsByClassName('btn-small')[0].classList.contains('disabled')){
            $rootScope.$broadcast('focus', 'drawer', keyCode, {x: 1840, y: 0, width: 0, height: 0});
          }else{
            $rootScope.$broadcast('focus', 'main', keyCode, '');
          }
        }
        break;
...
//        }
        ...
        document.querySelector("[depth3-ad]").querySelector(".ad-dim").style.visibility = 'hidden';
        document.querySelector("[depth3-ad]").style.visibility = 'hidden';
        setTimeout(function() {
          document.querySelector("[depth3-ad]").querySelector(".ad-dim").style.visibility = 'visible';
          document.querySelector("[depth3-ad]").style.visibility = 'visible';
        }, 1000);
        $scope.closeFullScreen();
...
//          $element[0].getElementsByTagName('video')[0].play();
  ...
  $scope.closeFullScreen = function () {
    ...
    /*
     ...
     setTimeout(function () {
     $scope.setFocusItem('depth3Ad', $element[0]);
     }, focusDelay);
     */
