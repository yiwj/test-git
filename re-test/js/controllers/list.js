...
app.controller('listController', function($scope, $controller, $element, $rootScope, $timeout, server, focusManager, keyHandler, marquee, util, storage, popupService, pmLog, eventKey, device, timeOutValue, adManager, watchProcess, audioGuidance) {
  ...
  $scope.setFocusItem = function(item, element) {
    ...
    if ($scope.focusItem === 'back') {
      ...
      // QEVENTSIXT-13610 : back key에 의한 default focus시 marquee 동작 불능 이슈
      marquee.setTarget(lastFocus.element.getElementsByClassName('marquee')[0]);
...
//spec out          } else if ((contentName === msgLang.tvshow_badge_youmaylike) &&
//            element.classList &&
...
//            params.text += title;
        ...
        params.text = msgLang.audio_2017_button_info || 'more information';
        audioGuidance.call(params);
      } else if($scope.listData.total === 0) {   // nodata 발화
        params.text = $scope.nodata;
        audioGuidance.call(params);
      }
    } else {
  ...
  $scope.executeAction = function(submenu) {
    // console.log('list.executeAction, submenu=' + submenu);
    ...
    if (focusObject.scope == $scope) {
      ...
      if (target == 'back') {
      ...
      } else if (target.indexOf('item') >= 0 || $rootScope.isPlayKey) {
        ...
        watchProcess.execProcess($scope.$id, module, item, 'List Page', $rootScope.pmLogValue, 'exec_full');
...
//              $scope.sort = $scope.sort + ', ' + obj.values[k].name;
    ...
    for (i = 0; i < l; i++) {
      ...
      if (obj.selected == 'TRUE') {
        ...
        if ($scope.selectedMenu &&
          ($scope.selectedMenu.indexOf('CP_') === 0)) {
          // [QEVENTSIXT-15267] 일부 국가에서 CP menu의 code값은 ME**** 형태가 아니다.
          // 예) GB에서 CP_MV_wuakiUK 메뉴
          $scope.selectedCPMenuCode = targetMenu;
        } else {
          $scope.selectedCPMenuCode = '';
        }
  ...
  $scope.requestDataASync = function(append) {
    // console.log('list.requestDataASync, append=' + append);
    util.async(function() {
      requestData(append);
    });
  };
  ...
  var requestMenu = function(type) {
    // console.log('list.requestMenu, type=' + type);
    var cat;
    if (type === 'tvshows') {
      cat = 'TS';
    } else {
...
//    if (focusManager.getState('option') == true) return;
  ...
  $scope.moveFocusByKey = function(keyCode, isPageUpDownByChKey) {
    // console.log('list.moveFocusByKey, keyCode=' + keyCode);
...
//  };
    ...
    switch (keyCode) {
      ...
      case keyHandler.RIGHT:
        if($scope.listData.total === 0) {
          moved = false;
          break;
        } else if ($scope._ctrls['tvshowsList'] && $scope._ctrls['tvshowsList'].scope) {
  ...
  //      } else {
    ...
    if (!update) {
      ...
      playArr = $element[0].getElementsByClassName('icon-code');
      l = playArr.length;
      for (i = 0; i < l; i++) {
