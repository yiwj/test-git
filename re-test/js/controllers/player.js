...
app.controller('playerController', function($scope, $controller, $element, $timeout, $rootScope, device, focusManager, keyHandler, marquee, util, getStatus, pmLog, appService, popupService, screenSaver, audioGuidance) {
  ...
  var owner = null;
  var player = null;
  var _imgPosterElement;
  var _slideScrollerElement;
  var _slideLastOnItemElement;
  var _ignoreKeyEventOnce;
  var _scrollPrevElement;
  var _scrollNextElement;
  var _slidePlayerControlElement;
  var _lastSelectedSlideItemIdx;
  ...
  var FEEDBACK_HIDE_TIMEOUT = 3000;
  ...
  $scope.lastLoadingAudioGuidance = undefined;

  var timeClickedOnProgressbar;

  $scope.hideScreenDim = true;
  $scope.playerControlStyle = {};
  ...
  var changeBtnNameToIdx = function (btnName) {
    switch (btnName) {
      case 'speed-fast':
        return 'speed-0';
      case 'speed-medium':
  ...
  $scope.recoverFocus = function(activeFocus) {
    ...
    if (lastActiveFocus && lastActiveFocus.item) {
      // [WOSLQEVENT-39111]
    ...
    if (name) {
      var element = '';
      if(name.indexOf('speed-') === -1) {     // [QEVENTSIXT-19212] speed 조절 팝업에서 마우스 커서 이동후 복귀시 포커스 복원
        element = $element[0].getElementsByClassName(name)[0];
      } else {
        element = lastActiveFocus.element;
        name = changeBtnNameToIdx(name);
      }
      if (element) {
  ...
  $scope.audioGuidance = function (scope, target, element) {
    ...
    switch (target) {
      ...
      case 'btnIcon-2':
        if ($scope.playControl === 'play') {
          ...
          if($scope.isFirstLoading) {
            txt = $scope.trailerData.title + '. ' + txt;
      ...
      case 'btnIcon-0':
        ...
        break;
      case 'btnPlayMovie':
        if ($scope.initParams.isTvShow) {
          txt = msgLang.player_button_playtvshow;
        } else {
    ...
    if (focusObject.scope == $scope) {
      ...
      if (!$scope.hideTrailerEndButtons) {
        ...
        if(target == 'btnPlayMovie') {
          // webOS3.5 pmLog 추가
          pmLog.write(pmLog.LOGKEY.TRAILER_ACTION, {
        ...
        if(target.indexOf('btnIcon') >= 0) {// control 영역
          if (target == 'btnIcon-0') {
            if (!$scope.slideHide) {
              // 하단의 slide 영역의 좌측 back 버튼이 클릭된 경우
              closeSlide();
            } else {
              // webOS3.5 pmLog 추가
              pmLog.write(pmLog.LOGKEY.TRAILER_ACTION, {
            ...
            _lastSelectedSlideItemIdx = $scope.trailer.idx;
            showSlide();
          }
        } else if(target == 'btnIcon-0') {
          closeSlide();
        ...
        if ($scope.controlOpen) {
          // control이 보여진 상태면, player 닫지 않고 control만 숨김 (executeByKey 에서)
        } else if ($scope.slideOpen) {
          // slide가 보여진 상태면, player 닫지 않고 slide만 숨김 (executeByKey 에서)
        } else {
          // webOS3.5 pmLog 추가
          pmLog.write(pmLog.LOGKEY.TRAILER_ACTION, {
            contents_id : $scope.itemId,
    ...
    if (keyCode === keyHandler.BACK) {
      if (!$rootScope.spinner.hide) {
        ...
        showSpeed();
          ...
          $scope.$apply();
          return;
      } else if ($scope.controlOpen) {
        // control이 보여진 상태면, control 숨기기
        $scope.headHide = true;
        $scope.headOpen = false;
        $scope.controlHide = true;
        $scope.controlOpen = false;
      ...
      if ($scope.controlHide && $scope.slideHide) {
        // slider 또는 control에 focus가 있는 상태에서, timeout으로 hide가 된 이후,
        // 리모콘의 버튼이 눌린 경우는, control / feedback을 보여주도록
        if($scope.isMovie) {
          showFeedbackPopup();
        } else {
          selectControlPopup();
  ...
  $scope.closePlayer = function() {
    _slideLastOnItemElement = undefined;
    ...
    _imgPosterElement.setAttribute('style', 'display: block;');
    $scope.hideTrailerEndButtons = true;
  ...
  var initMouseEvents = function() {
    ...
    $scope.setMouseEvent(_imgPosterElement);
  ...
  var setPanel = function(idx) {
    ...
    if($scope.trailer.currentPanelData.type === 'V') {
      ...
      util.async(function() {
        ...
        _imgPosterElement.style.backgroundImage = 'url(' + url + ')';

        if (_slideLastOnItemElement) {
          _slideLastOnItemElement.classList.remove('on');
          _slideLastOnItemElement = undefined;
        }

        var elements = _slideScrollerElement.getElementsByClassName('item-' + $scope.trailer.idx);
        if (elements && (elements.length > 0)) {
          elements[0].classList.add('on');
          _slideLastOnItemElement = elements[0];
        }
      });
    }
  };
  ...
  var showSlide = function() {
    ...
    if($scope.slideHide) {
      ...
      util.async(function() {
        ...
        // slide item들이 scroll영역에 생성될 시간이 필요
        setTimeout(updateScrollBtnStatus, 400);
      });
    } else {
      // slide 숨기기
      $scope.slideHide = true;
  ...
  var showFeedbackPopup = function(feedbackControl) {
    if($scope.controlHide && $scope.slideHide) {
      if(feedbackControl) {
        $scope.feedbackControl = feedbackControl;
        $scope.controlTick = videoHelper.makeDurationTime2DisplayTime($scope.currentTime);
        $scope.begintTick = $scope.controlTick;
        tellPointer();
      }
    ...
    if(($scope.controlHide || $scope.headHide) && $scope.slideHide) {
      ...
      util.async(function() {
        ...
        if(!window.PalmSystem || !window.PalmSystem.cursor.visibility) {
          // Play 버튼에 Focus
          var name = 'btnIcon-2';
          if (lastFocus && lastFocus.item && (lastFocus.item.indexOf('btnIcon-') >= 0)) {
            name = lastFocus.item;
          }
  ...
  // slide show control
  var slideHelper = {
    setSlideSpeed : function() {// 선택된 속도로 슬라이드쇼 실행
    ...
    resetSlideSpeed : function() {// 슬라이드쇼 실행
      ...
      if ($scope.trailer.slideShow.active) {
        $scope.trailer.slideShow.slideInterval = setInterval(function () {
          // console.log('player.slideHelper.resetSlideSpeed.interval.inner begin');
          if (!$scope.trailer.slideShow.active) {
            return;
          }
    ...
    clearSlideShow : function() {// 슬라이드쇼 cancel
      ...
      if ($scope.trailer.slideShow.slideInterval) {
        clearInterval($scope.trailer.slideShow.slideInterval);
  ...
  var timeoutHelper = {// header, control, slide, slideshow popup timeout
    ...
    feedbackTimeout: function() {
      var timeout = FEEDBACK_HIDE_TIMEOUT;

      $timeout.cancel(_feedbackTimeout);
      _feedbackTimeout = $timeout(function() {
        // console.log('player.controlTimeout, inner');
        $scope.feedbackHide = true;
        $scope.feedbackOpen = false;
        $scope.$apply();
      ...
      if ($scope.currentTime >= $scope.videoEndTime) {
        ...
        if (util.isAWSServer()) {
          ...
          if (device.isAudioGuidance &&
            ...
            $scope.lastLoadingAudioGuidance = new Date();

            var speech = msgLang.audio_content_loading;
            speech += '. ';
            speech += $scope.controlTick;
  ...
  $scope.moveFocusByKey = function(keyCode) {
    ...
    // console.log('player.moveFocusByKey, focusItem=' + focusItem + ', keyCode=' + keyCode + ', _ignoreKeyEventOnce=' + _ignoreKeyEventOnce);

    if (_ignoreKeyEventOnce) {
      // focus를 되살리는 key stroke 였음.
      _ignoreKeyEventOnce = false;
      return;
    }
    ...
    if($scope.slideOpen) {
      if($scope.speedOpen &&
        ...
        // 하단 slide 영역에 control button들이 추가됨.
  ...
  var moveFocusFromDefault = function (keyCode) {
    ...
    var curIdx, itemCnt, focusItem, itemName, scrollX, hidden, name, element, moveType;
    if (keyCode === keyHandler.UP || keyCode === keyHandler.DOWN) {
      ...
      if($scope.trailer.currentPanelData.type === 'V') {
        ...
        if (($scope.currentTime + 10) < $scope.videoEndTime) {
          ...
          if($scope.playControl === 'pause') {    // play중
            moveType = 'jumpforward';
          } else {
            moveType = 'pausejumpforward';
          }
          showFeedbackPopup(moveType);
        } else {
      ...
      if($scope.trailer.currentPanelData.type === 'V') {
        ...
        if (($scope.currentTime - 10) >= 0) {
          ...
          if($scope.playControl === 'pause') {    // play중
            moveType = 'jumpbackward';
          } else {
            moveType = 'pausejumpbackward';
          }
          showFeedbackPopup(moveType);
        } else {
          $scope.currentTime = 0;
  ...
  var selectImageFromSlide = function(index, moveLeft) {
    ...
    if (name && element) {
      _lastSelectedSlideItemIdx = index;
  ...
  var moveFocusFromSlide = function(keyCode) {
    ...
    // isRTL이 true인 경우
    {
      'from': 'btnSpeed',
      'to': {
        'right': {'valid': device.isRTL, 'name': 'btnIcon-1'}
      }
    }, {
      'from': 'btnIcon-1',
      'to': {
        'left': {'valid': device.isRTL, 'name': 'btnSpeed'},
        'right': {'valid': device.isRTL, 'name': 'btnIcon-2'}
      }
    }, {
      'from': 'btnIcon-2',
      'to': {
        'left': {'valid': device.isRTL, 'name': 'btnIcon-1'},
        'right': {'valid': device.isRTL, 'name': 'btnIcon-3'}
      }
    }, {
      'from': 'btnIcon-3',
      'to': {
        'left': {'valid': device.isRTL, 'name': 'btnIcon-2'},
        'right': {'valid': device.isRTL, 'name': 'btnIcon-0'}
      }
    }, {
      'from': 'btnIcon-0',
      'to': {
        'left': {'valid': device.isRTL, 'name': 'btnIcon-3'},
        'right': {'valid': device.isRTL, 'doNothing': true}
      }
    },
    ...
    // isRTL이 false인 경우
    {
      ...
      'to': {
        'left': {'valid': !device.isRTL, 'name': 'btnIcon-3'},
        'down': {'moveToSlideItem': true, 'from': 'btnSpeed'}
      }
    }, {
      'from': 'btnIcon-3',
      'to': {
        'left': {'valid': !device.isRTL, 'name': 'btnIcon-2'},
        'right': {'valid': !device.isRTL, 'name': 'btnSpeed'},
        'down': {'moveToSlideItem': true, 'from': 'button'}
      }
    }, {
      'from': 'btnIcon-2',
      'to': {
      ...
      'from': 'btnIcon-1',
      'to': {
        'left': {'valid': !device.isRTL, 'name': 'btnIcon-0'},
        'right': {'valid': !device.isRTL, 'name': 'btnIcon-2'},
        'down': {'moveToSlideItem': true, 'from': 'button'}
      }
    }, {
    ...
    for (i = 0; i < focusMap.length; i++) {
      if (focusMap[i].from === focusItem) {
        var moveFrom = '', moveTo = '';
        var moveToSlideItem = false;
        switch (keyCode) {
          case keyHandler.LEFT:
            if (focusMap[i].to.left && focusMap[i].to.left.valid) {
              if (focusMap[i].to.left.doNothing) {
                return;
              }
              moveTo = focusMap[i].to.left.name;
            }
            break;
          case keyHandler.RIGHT:
            if (focusMap[i].to.right && focusMap[i].to.right.valid) {
              if (focusMap[i].to.right.doNothing) {
                return;
              }
              moveTo = focusMap[i].to.right.name;
            }
            break;
          case keyHandler.DOWN:
            if (focusMap[i].to.down) {
              moveToSlideItem = focusMap[i].to.down.moveToSlideItem;
              moveFrom = focusMap[i].to.down.from;
            }
            break;
        }

        if (moveTo) {
          element = $element[0].getElementsByClassName(moveTo)[0];
          $scope.setFocusItem(moveTo, element);
          return;
        } else if (moveToSlideItem) {
          // button → slide list item
          $scope.focusFromScroll(moveFrom);
          return;
        }
      }
    }
    ...
    if (keyCode == keyHandler.UP) {
      if(!$scope.speedOpen && focusItem == 'btnSpeed') {
        ...
        $scope.focusFromSlideItem();
      } else if(focusItem == 'scroll-prev' || focusItem == 'scroll-next'){// scroll → item
        $scope.focusFromScroll(focusItem);
      }
    } else if (keyCode == keyHandler.DOWN) {
      if(focusItem == 'btnSpeed' || focusItem == 'btnIcon-0') {
        // button → slide list item
        $scope.focusFromScroll('button');
      } else if($scope.scrollHide) {
        return;
      } else if(itemName == 'item') {// slide list item → scroll 영역
        if (!isScrollFocusable()) {
          // console.log('player.moveFocusFromSlide, slide not exist');
          return;
        }
        $scope.focusToScroll();
      }
    } else if (keyCode == keyHandler.RIGHT) {
      if(focusItem == 'btnSpeed') {
        // speed button이 가장 오른쪽에 위치함
        return;
      } else if(focusItem == 'scroll-prev') {
        if (_scrollNextElement.classList.contains('disabled')) {
          return;
        }
        $scope.setFocusItem('scroll-next', _scrollNextElement);
      ...
      if(focusItem == 'btnIcon-0') {
        // back button이 가장 왼쪽에 위치함
      } else if(focusItem == 'scroll-next') {
        if (_scrollPrevElement.classList.contains('disabled')) {
          return;
        }
        $scope.setFocusItem('scroll-prev', _scrollPrevElement);
      } else if(focusItem == 'btnIcon-0' || focusItem == 'scroll-prev') {
        return;
      } else {// slide list 영역
  ...
  var moveFocusFromControl = function(keyCode) {
    ...
    if (keyCode == keyHandler.UP) {
      if ($scope.isMovie &&
        ((focusItem === 'btnIcon-0') || (focusItem === 'btnIcon-2'))) {
  ...
  $scope.focusFromSlideItem = function() {
    // console.log('player.focusFromSlideItem');
    var name, element, i, btn, l, min, gap;
    var btnCenter = focusElement.getBoundingClientRect().left + (focusElement.getBoundingClientRect().width /2);

    if (!_slidePlayerControlElement) {
      _slidePlayerControlElement = $element[0].getElementsByClassName('player-control')[0];
    }

    var classNames = ['btn-large', 'icon-player'];
    for (var j = 0 ; j < classNames.length ; j++) {
      var elements = _slidePlayerControlElement.getElementsByClassName(classNames[j]);
      for(i = 0; i < elements.length; i++) {
        btn = elements[i];
        var itemCenter = btn.getBoundingClientRect().left + (btn.getBoundingClientRect().width /2);
        gap = Math.abs(btnCenter - itemCenter);
  ...
  $scope.focusFromScroll = function(target) {
    // console.log('player.focusFromScroll');
    var elementLeft, scrollX, min, gap, moveToName, hidden, name, element;

    if (_lastSelectedSlideItemIdx !== undefined) {
      // 이전 slide item 선택으로 복귀
      var moveLeft;
      name = 'item-' + _lastSelectedSlideItemIdx;
      element = $element[0].getElementsByClassName(name)[0];
      selectImageFromSlide(_lastSelectedSlideItemIdx, element.getBoundingClientRect().left < 0);
      return;
    }
  ...
  var isScrollFocusable = function() {
    if (!$scope.scroll || !$scope.scroll.maxScrollX) {
      // scroll 없음, item 개수가 작을 때
      return false;
    }

    if (_scrollPrevElement.classList.contains('disabled') &&
      _scrollNextElement.classList.contains('disabled')) {
      return false;
    }
  ...
  $scope.focusToScroll = function() {
    ...
    if (x < areaX) {
      name = 'scroll-prev';
      element = _scrollPrevElement;
    } else {
      name = 'scroll-next';
      element = _scrollNextElement;
    }

    if ((name === 'scroll-prev') && _scrollPrevElement.classList.contains('disabled')) {
      name = 'scroll-next';
      element = _scrollNextElement;
    } else if ((name === 'scroll-next') && _scrollNextElement.classList.contains('disabled')) {
      name = 'scroll-prev';
      element = _scrollPrevElement;
    }
  ...
  $scope.initPlay = function(scope, initParams, data, fromTrailerBtn, itemId, execList, onPageMoveCallback) {
    ...
    _imgPosterElement = $element[0].getElementsByClassName('player_thumb')[0];
    _slideScrollerElement = $element[0].getElementsByClassName('player-scroller')[0];
    _slideLastOnItemElement = undefined;
    _lastSelectedSlideItemIdx = undefined;
    lastFocus = {};
    lastActiveFocus = {};
    focusManager.setCurrent($scope, '');
    ...
    if($scope.trailerData &&
      ...
      // 이미지일 경우, 하단 control 영역의 bg color 설정
      $scope.playerControlStyle = {'background-color': 'rgba(24,24,24,0.2)'};
    } else {
      // Video일 경우, 하단 control 영역의 bg color는 없음
      $scope.playerControlStyle = {};
    }
  ...
  var isSlideItemVisible = function(idx) {
    if ((idx < 0) || (idx >= $scope.trailerData.data.length)) {
      return false;
    }

    var elements = _slideScrollerElement.getElementsByClassName('item-' + idx);
    if (!elements || elements.length < 1) {
      return false;
    }

    var itemElement = elements[0];
    if (itemElement.getBoundingClientRect().right < 0) {
      return false;
    } else if ((scroll.wrapperW < itemElement.getBoundingClientRect().left)) {
      return false;
    }

    var scrollX = itemElement.getBoundingClientRect().left - scroll.wrapperOffsetLeft - scroll.x;
    if (scrollX < -scroll.x) {
      return false;
    }

    scrollX = itemElement.getBoundingClientRect().left + itemElement.offsetWidth - scroll.wrapperOffsetLeft - scroll.x;
    if ((scrollX - 1) > (scroll.wrapperW - scroll.x)) {
      // -1을 하는 이유는, 소수점 아래 숫자는 무시하기 위해
      return false;
    }
  ...
  var updateScrollBtnStatus = function() {
    var firstItemVisible = isSlideItemVisible(0);
    var lastItemVisible = isSlideItemVisible($scope.trailerData.data.length - 1);

    if (_slideScrollerElement.getBoundingClientRect().height < 1) {
      // console.log('player.updateScrollBtnStatus, scroll not visible');
      return;
    }

    // console.log('player.updateScrollBtnStatus, firstItemVisible=' + firstItemVisible +
    //   ', lastItemVisible=' + lastItemVisible);

    if (firstItemVisible) {
      // 보이면, scroll 버튼이 작동할 필요가 없음
      _scrollPrevElement.classList.add('disabled');
    } else if (_scrollPrevElement.classList.contains('disabled')) {
      _scrollPrevElement.classList.remove('disabled');
    }

    if (lastItemVisible) {
      _scrollNextElement.classList.add('disabled');
    } else if (_scrollNextElement.classList.contains('disabled')) {
      _scrollNextElement.classList.remove('disabled');
    }
  };

  var end = function(x) {
    if (_slideScrollerElement.getBoundingClientRect().height < 1) {
      // console.log('player.end, scroll not visible');
      return;
    }

    // console.log('player.end, x=' + x +
    //   ', $scope.scroll.x=' + $scope.scroll.x +
    //   ', $scope.scroll.maxScrollX=' + $scope.scroll.maxScrollX +
    //   ', $scope.focusItem=' + $scope.focusItem);

    updateScrollBtnStatus();

    var name, element;
    if (($scope.scroll.x === 0) && ($scope.focusItem === 'scroll-prev')) {
      // prev 버튼을 클릭하여, 왼쪽 끝으로 이동한 경우, next 버튼에 focus가 가도록
      _scrollNextElement.classList.remove('disabled');
      _scrollPrevElement.classList.add('disabled');
      $scope.setFocusItem('scroll-next', _scrollNextElement);
      return;
    } else if (($scope.scroll.x === $scope.scroll.maxScrollX) && ($scope.focusItem === 'scroll-next')) {
      // next 버튼을 클릭하여, 오른쪽 끝으로 이동한 경우, prev 버튼에 focus가 가도록
      _scrollNextElement.classList.add('disabled');
      _scrollPrevElement.classList.remove('disabled');
      $scope.setFocusItem('scroll-prev', _scrollPrevElement);
      return;
  ...
  var initializeScroll = function() {
    ...
    if (!_scrollPrevElement) {
      _scrollPrevElement = $element[0].getElementsByClassName('scroll-prev')[0];
    }
    if (!_scrollNextElement) {
      _scrollNextElement = $element[0].getElementsByClassName('scroll-next')[0];
    }
  };
  ...
  $scope.setDefaultFocus = function() {
    ...
    if(!$scope.speedHide) {
      name = 'speed-' + $scope.trailer.slideShow.nameMap[$scope.trailer.slideShow.currentSpeed].idx;
      target = $element[0].getElementsByClassName(name)[0];
    } else {
  ...
  $scope.onCursorVisibilityChanged = function(visible) {
    ...
    if (visible) {
      ...
      // focus만 살리고, key 처리는 하지 않도록
      _ignoreKeyEventOnce = true;
      $scope.recoverFocus();
    }
    timeoutHelper.restartTimeout();
  };
        ...
        if (this.lastVideoTime && !this.bufferingTimer) {
          this.bufferingTimer = setTimeout(function() {
            ...
            if ($scope.$parent.open &&
              ...
              var skip;
              var diff;
              if ($scope.$parent.lastLoadingAudioGuidance) {
                diff = new Date() - $scope.$parent.lastLoadingAudioGuidance;
              }

              // console.log('player, diff=' + diff);
              if (diff && (diff < 2000)) {
                // console.log('player, skipAudioGuidance 2');
                // 여러 차례 loading을 발화하지 않도록
                skip = true;
              }

              $scope.$root.spinner.showSpinner({
                skipAudioGuidance: skip
              });
            } else {
    ...
    link: function (scope, elm) {
      scope.$watch('videoCurrentTime', function (newVal) {
        ...
        if(elm[0].isPlayControl) {// progressbar click 시 해당 지점으로의 이동
          ...
          // console.log('player, diff=' + diff);
          if (diff && (diff < 2000)) {
            // console.log('player, skipAudioGuidance 1');
            // 여러 차례 loading을 발화하지 않도록
            skip = true;
          }

          scope.$root.spinner.showSpinner({
            audioGuidanceForce: true,
            skipAudioGuidance: skip
          });
