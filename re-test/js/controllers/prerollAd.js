...
app.controller('prerollAdController', function($rootScope, $scope, $controller, $element, $rootScope, $sce, $timeout, $compile, focusManager, keyHandler, marquee, adManager, audioGuidance, util, pmLog) {
  ...
  $scope.prerollPlay = function(data, callback) {
...
//    }
    ...
    if (prerollData) {
      // webOS3.5 pmLog 추가
      pmLog.write(pmLog.LOGKEY.PREROLL_DISPLAY, {
      ...
      playTimeIntervalID = setInterval(function(){
        ...
        //[QEVENTSIXT-19759] - 광고 영상 중 buffering 상태(readyState = 2) 일 때 로딩 바 표시 함
        //readyState = 0 - no information whether or not the audio/video is ready
        //readyState = 1 - metadata for the audio/video is ready
        //readyState = 2 - data for the current playback position is available, but not enough data to play next frame/millisecond
        //readyState = 3 - data for the current and at least the next frame is available
        //readyState = 4 - enough data available to start playing
        if ($this.readyState == 2) {
          if ($rootScope.spinner.hide) {
            $rootScope.spinner.showSpinner();
            if ($element[0].querySelector('#skipbtn').classList.contains('focus')){
              $element[0].querySelector('#skipbtn').classList.remove('focus');
            }
          }
        }else {
          if (!$rootScope.spinner.hide) {
