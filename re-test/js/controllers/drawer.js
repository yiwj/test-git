...
app.controller('drawerController', function($scope, $controller, $element, $rootScope, $timeout, server, focusManager, device, keyHandler, storage, pmLog, eventKey, searchManager, audioGuidance, marquee) {
  ...
  var drawIcon = function(e, response) {
    ...
    response.menuList = response.menuList.filter(function (el) {
      return (el.serviceCode !== '3d' && el.serviceCode !== 'premium');
    });
  ...
  $scope.setFocusItem = function(item, element) {
    ...
    if (item != '') {
      ...
      if (element) {
        ...
        var x = 0; y = 0;
        if (item !== 'close') {
          x = element.offsetLeft + 50 * zoomRatio;
          y = element.offsetTop + 108 * zoomRatio;
        } else {
  ...
  $scope.executeAction = function() {
    ...
    if (focusObject.scope == $scope) {
      ...
      if (target != device.currentPage) {
        if (target !== 'search'){
          $scope.isDrawerLink = true;
          //[QEVENTSIXT-32167] _drawer.drawerLastMenu가 'search'로 되어 있어 API 호출 할 때 default인 TV로 호출하는 문제처리
  ...
  var focusHandler = function(e, target, option, rect) {
    ...
    if ($rootScope.pageManager.peekHistory() && $rootScope.pageManager.peekHistory().page === "featured") {
      for (i = 0; i < arr.length; i++) {
        if (device.isRTL) {
          var centerPos = arr[i].getBoundingClientRect().right + arr[i].getBoundingClientRect().width / 2.0;
        } else {
