...
//    document.body.classList.remove('hover-mode');
  ...
  var itemMouseEnter = function(e) {
    ...
    if(this.classList.contains('btn-cp-play')){
      //mainpage에서 컨텐츠의 i 이미지 올 때 cp-hover 추가 해야 poster div에 focus가 중복으로 들어가지 않게 함
      if(this.classList.contains('icon-code')){
        this.parentElement.parentElement.classList.add('cp-hover');
      }else{
        this.parentElement.classList.add('cp-hover');
      }
    }
...
//    else document.body.classList.remove('hover-mode');
    ...
    //메인페이지 tier1의 cp 선택 버튼 mouse leave 일 경우 상위 element cp-hover 제거 (parentElemnet의 parentElemnet 영역에 있음 )
    if(this.classList.contains('btn-cp-play') && this.parentElement.parentElement.classList.contains('cp-hover')) this.parentElement.parentElement.classList.remove('cp-hover');
    e.stopPropagation();
