...
app.controller('appListController', function($scope, $controller, $element, $rootScope, $timeout, server, focusManager, keyHandler, marquee, util, storage, pmLog, eventKey, device, timeOutValue, adManager, audioGuidance) {
  ...
  $scope.setFocusItem = function(item, element) {
    ...
    //2017년향 html변경으로인한 포커스 소스 수정
    focusElement = element;

    //item 를 정상적으로 가지고 오나 element를 가지고 오지 못함.
    var elementItem,elCheck1;
    
    if (item && !element) {
      elCheck1 = $element[0].querySelector('[item="'+item+'"]').children;
      element = elCheck1;
    }
    ...
    if (item) {
      if(focusElement.getElementsByClassName('marquee')[0]){
        marquee.setTarget(focusElement.getElementsByClassName('marquee')[0]);
      }
      if(focusElement.getElementsByClassName('marquee')[1]){
        marquee.setTarget2(focusElement.getElementsByClassName('marquee')[1]);
      }
  ...
  $scope.audioGuidance = function (scope, target, element) {
    ...
    if (element) {
      if (util.isAWSServer()) {
        ...
        if (element.querySelector('.text') && element.querySelector('.text').parentElement.classList.contains('badge-text')) {
          ...
          //[QEVENTSIXT-5376] app list에서 app title과 category 아래 Free 문구 출력 안됨
          //카테고리 발화
          appCategory = element.querySelector('.focus .app-category');
          if(appCategory) {
            appCategory = appCategory.innerText;
          }
          //가격 발화
          itemPrice = element.querySelector('.focus .item-price');
          if(itemPrice) {
    ...
    if(appCategory){
      ...
      contentName += appCategory;
    }
    if(itemPrice){
      contentName += '. ';
      contentName += itemPrice;
    }
  ...
  $scope.onInitialized = function(name, scope) {
    ...
    /*
    수동편성 banner 중 Entertainment 는 subCategory 까지 지정해줘야 하기때문에
    $scope.selectedCategory를 따로 설정해줌
    단, 인기/최신 수동 banner 는 subCategory 지정이 필요 없으므로
    flag 변수를 통해 제어 함.
    */
    if (device.isEntertainment) {
      $scope.selectedCategory = '003';
      $element[0].setAttribute('item', '5');
      device.isEntertainment = false;
    }
  ...
  var onDraw = function(e, response) {
    ...
    e.preventDefault();
    //[QEVENTSIXT-19168] 목록 진입시 설정한 별점 표시 안됨
    // Breadcrumb을 통해서 앱리스트를 진입했을 경우 이전 데이터로 response를 설정해주지 못하게 수정
    device.prevListData = response;
    // from back
    ...
    // 장르 클릭 시 로깅되지 않는 이슈 (executeAction에 있는 것 주석처리) - 로딩 시 제외
    if (!isFirst) {
      pmLog.write(pmLog.LOGKEY.SECOND_MENU_CLICK, {
  ...
  var defaultSelectedMenu = function(listData) {
    var i, l, obj;
    if(!listData.menuList) return;
    l = listData.menuList.length;
    for (i = 0; i < l; i++) {
        obj = listData.menuList[i];
        if($rootScope.selectedMenuCode && $rootScope.selectedMenuCode === obj.code) {
            obj.selected = 'TRUE';
        }
    }
  };
  ...
  var onMenuLoaded = function(e, response) {
    ...
    if (response.menuList === undefined) {
      ...
      if ($scope.listData) {
        ...
        //promotion banner관련 카테고리 메뉴 기본 설정
        if($rootScope.selectedMenuCode) {
          defaultSelectedMenu($scope.listData);
          $rootScope.selectedMenuCode = "";
        }
      }
  ...
  var requestData = function(append) {
    ...
    //[QEVENTSIXT-12090] All - Game category에 진입하여 확인시 해당 App 없음
  ...
  $scope.setDefaultFocus = function() {
    ...
    //[QEVENTSIXT-18095][QEVENTSIXT-18175] Focus ui 사라지고, 4방향키 입력시에도 다시 생성 안됨
    if(!target){
      target = $element[0].querySelector('.item0-0');
    }
  ...
  $scope.focusToHeader = function(rect) {
    ...
    //[QEVENTSIXT-23643] 문구 뒤에 ... 붙은 상태로 계속 marquee 동작함
    marquee.setTarget(null);
    marquee.setTarget2(null);
  ...
  var focusHandler = function(e, target, keyCode, rect) {
    ...
    if (_ctrls['appngameList'] && _ctrls['appngameList'].scope) {
      //[QEVENTSIXT-22152] RTL국가 일 때 우측 Category list로 포커스 이동 안하는 이슈 처리.
      if (!device.isRTL){
        ...
        if($element[0].getElementsByClassName('panel-body')[0].offsetLeft < rect.x) {
          if(keyCode === keyHandler.UP) _ctrls['appngameList'].scope.focusFromAD(rect);
          else if (keyCode === keyHandler.DOWN) _ctrls['appngameList'].scope.focusFromScroll('prev', true, lastItemFocus, lastItemMenuFocus);
          else _ctrls['appngameList'].scope.focusFromScroll('prev', true, lastItemFocus); // depth2Ad -> _KEYCODE.RIGHT
        } else {
          _ctrls['appngameList'].scope.focusFromScroll('prev', true, lastItemFocus, lastItemMenuFocus);
        }
      }
...
//    console.log('device.param ::::::::::::::::' + JSON.stringify(device.param));
  ...
  $scope.initialize = function() {
    ...
    //[QEVENTSIXT-4540] 앱 리스트 페이지 진입시 스크롤 prev가 활성화 되는 이슈 처리(M2 보드일 때만 타도록 함)
