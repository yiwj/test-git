app.controller('listControl', function($scope, $controller, $element, $attrs, $timeout,
  ...
  $scope.initializeList = function(callback) {
    ...
    if (isAppnGame()) {
      if (isMyPage())
        ...
        $scope.nodata = msgLang.list_2017_noapp_1;
    }
    ...
    for (i = 0; i < l; i++) {
      ...
      if (needMouseEvent) {
        ...
        //[QEVENTSIXT-21501] 메뉴 및 Sub category 이름은 읽어주나 정작 읽어야 할 포커스된 컨텐츠는 발화하지 않음.
        //listAppItem.js에서 $scope.$digest() 뺀 로직 추가
        $scope.$digest();
        for (; j < $scope.itemPerRow; j++) {
          arr[j] = -1;
          view = $element[0].getElementsByClassName('item' + obj.id + '-' + j)[0];
  ...
  $scope.executeAction = function(params) {
    // console.log('list.executeAction, params=' + params);
    ...
    if(params && params.target &&
      ...
      params.target &&
      params.focusElement &&
      params.focusElement.classList &&
      params.focusElement.classList.contains('btn-cp-play')) {
      ...
      var item = params.target;  //params.focusElement.getAttribute('item');

      if (item === 'player' &&
        params.focusElement.parentElement) {
        // [QEVENTSIXT-27563] cursor로 클릭하는 경우와 매직리모콘으로 클릭하는 경우
        // focusElement가 다름
        item = params.focusElement.parentElement.getAttribute('item');
        if (!item &&
          params.focusElement.parentElement.parentElement) {
          item = params.focusElement.parentElement.parentElement.getAttribute('item');
        }
      }
      ...
      //[QEVENTSIXT-15267] TV show/ movies에서 앱상세로 진입할 경우 Param변수 설정함.
      $rootScope.pageManager.setParam('scrollY', $scope.scroll.y);
      $rootScope.pageManager.setParam('itemId', itemId);
  ...
  $scope.moveFocusByKey = function(focusElement, focusItem, keyCode, isPageUpDownByChKey) {
    // console.log('listControl.moveFocusByKey, keyCode=' + keyCode);
    ...
    var marqueeTarget = marquee.getTarget(); // [QEVENTSIXT-20012] marquee 복원을 위해 이전 marquee를 저장
    // [QEVENTSIXT-13330] focus 벗어나도 marquee가 계속 동작 방지
    ...
    switch (keyCode) {
      ...
      case keyHandler.UP:
        ...
        if (scrollY < 0) {
          if ($scope.$parent.focusToHeader && isMyPage()) {
            ...
            return;
          } else if(!focusElement.classList.contains('btn-cp-play')) {
            if (isPageUpDownByChKey && isPageUpDownByChKey.isFrom) return;
        ...
        if (isAppnGame()) {
          ...
          if(focusElement.classList.contains('btn-cp-play')){
            moveScroll = false;
            name = 'item' + (row % visibleRowCount) + '-' + index;
          } else {
            moveScroll = true;
        ...
        element = $element[0].getElementsByClassName(name)[0];
        if (scrollY < -$scope.scroll.y && moveScroll === true) {
          isHiddenBefore = true;
          itemBefore = item;
          hidden = true;
          scrollY = -scrollY;
      ...
      case keyHandler.RIGHT:
        ...
        if (isAppnGame()) {
          ...
          if (index >= ($scope.itemPerRow-1) || $scope['row' + row][index + 1] === -1) {
            toScroll = true;
          }
        }
        if (toScroll) {
          ...
          if ($scope.scroll.wrapperH >= $scope.scroll.scrollerH) {
            //QEVENTSIXT-4385 scroll 버튼 비활성에서 focus가 item 3열에 위치 상태에서 right키 인가 시 동작 하지않게 주석 처리
            /*if (isMyPage()) {
              $scope.$parent.focusToHeader({x: focusElement.offsetLeft + 395, y: 0, width: 0, height: 0}, keyCode);
            }*/
            return;
          }
          //[QEVENTSIXT-4387] mypage 에서 첫번째 행을 제외한 나머지 행에서는 전부 down scroll 버튼으로 포커스 이동 됨.
          rect = {
            x: 0,
            y: focusElement.parentElement.offsetTop + $scope.scroll.y + 140,
            width: 0,
      ...
      case keyHandler.DOWN:
        ...
        if (isAppnGame()) {
          if (scrollY < 0) {
            marquee.setTarget(marqueeTarget);    // [QEVENTSIXT-20012] item이 맨 아래인 경우 marquee를 다시 복원
            return;
          }
          ...
          if(focusElement.classList.contains('btn-cp-play')){         // info icon에서 아래로 이동
            if (scrollY < 0) return;
            moveScroll = true;
            name = 'item' + ((row + 1) % visibleRowCount) + '-' + index;
            element = $element[0].getElementsByClassName(name)[0];
          } else {                                                    // 썸네일에서 내부의 info icon으로 이동
            moveScroll = false;
            element = element.getElementsByClassName('icon-code')[0];
          }
        }

        if (scrollY > $scope.scroll.wrapperH - $scope.scroll.y && moveScroll === true) {
          isHiddenBefore = true;
          itemBefore = item;
          hidden = true;
  ...
  $scope.restoreScrollPos = function(reset) {
    ...
    if (param &&
      (param.module === $scope.$parent.selectedMenu ||
      param.module === $scope.$parent.selectedCPMenuCode)) {
  ...
  $scope.isPromotion = function(r) {
    var view = $element[0].getElementsByClassName('row' + r)[0];
    var objType = view.getAttribute('data-type');
    if (objType === 'promotion') {
      return 'list-cont-promotion';
    }else {
      return 'list-cont-apps';
    }
  };
  ...
  $scope.initialize = function() {
    ...
    if ($attrs.itemh) {
      ...
      if (device.isHD && isAppnGame()) {
        //17년향 수정 [QEVENTSIXT-6544] APPS 아이콘들 겹쳐서 출력됨
        if($attrs.name === 'myappList'){
          itemH = 103;
        }else {
          itemH = 307;
          promotionH = 200;
