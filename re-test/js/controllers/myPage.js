...
app.controller('myPageController', function($scope, $controller, $element, $rootScope, $timeout, server, focusManager, membership, device, app, getAppInfo, keyHandler, marquee, util, storage, pmLog, appInstall, tvInstalledList, eventKey, timeOutValue, audioGuidance) {
  ...
  $scope.nodata = msgLang.mypage_2017_noinstall_1;
  ...
  $scope.setFocusItem = function(item, element) {
    ...
    if (util.isAWSServer()) {
      ...
      if ($scope.selectedFilter === '002') {
        if ($scope.hideListContent &&
          ...
          return;
        } else if (!$scope.hideListContent && (item === 'signin')) {
    ...
    //2017년향 템플릿 변경으로인한 포커스 소스 수정
    if(element && element.children){
      if (!element.getAttribute('item-id') && angular.element(element.children).hasClass('item-apps')) {
        focusElement = element.children[0];
  ...
  $scope.audioGuidance = function (scope, target, element) {
    ...
    if (target === 'option') {
      ...
      if (element && element.querySelector('.focus .text')) {
        ...
        if($scope.listData.itemList && ($scope.listData.itemList).length === 0 && $scope.focusItem === '1') {
          var tmpElement = scope.getScopeElement()[0];
          if (tmpElement.querySelector('.panel-header .text') && tmpElement.querySelector('.panel-header .text').innerText.length > 0) {
            contentName = tmpElement.querySelector('.panel-header .text').innerText + '. ' + contentName;
          }
          contentName +=  '. ' +msgLang.mypage_2017_noinstall_1;
        }
      }
  ...
  $scope.recoverFocus = function(item) {
    ...
    if (item) {
      // controller.js의 focus 이벤트에서 호출됨
      // LGStore app이 background에 있는 상태에서, app이 삭제되면,
      // controller가 blur 이벤트 발생시 저장했던 saveFocus.item이
      // 더이상 존재하지 않게 되는 경우 focus가 없어진다.
      var useDefault = false;
      var tokens = item.replace('item', '').split('-');
      if (tokens.length === 2) {
        // 실제 element가 있는지 확인
        var row = parseInt(tokens[0], 10);
        var col = parseInt(tokens[1], 10);
        var arr = $element[0].getElementsByClassName('list-cont');
        if (row < arr.length) {
      ...
      if (useDefault) {
        // default item으로 이동
        item = listItemTypeClass();
      }

      var target = $element[0].getElementsByClassName(item)[0];
      if (target) {
        recoverItem = {
      ...
      if (recoverItem) {
        $scope.setFocusItem(recoverItem.item, recoverItem.element);
      } else {
        // item이 없으면, 메뉴로 이동
        $scope.focusToMenu({x: 0, y: 0, width: 0, height: 0});
      }
    } else if (lastFocus.item && lastFocus.element) {
  ...
  $scope.executeAction = function() {
    ...
    if (focusObject.scope == $scope) {
      ...
      if (target == 'back') {
        ...
        // [QEVENTSIXT-3471]업데이트할 앱이 있으며 updateall 버튼 클릭 후 menu list(마이앱)으로 가도록 함
        moveFocusToItem('updateall', 'menu', keyHandler.LEFT);
        //[QEVENTSIXT-5774] $scope.listData.itemList의 bPremium,purchased등 response값이 빠져 처리 부분 주석 처리함
        //update all 버튼 누를 시 비로그인 시 멤버쉽 페이지로 이동하는 현상 수정
        appUpdateAct();
        /*
      ...
      if (purchasNoinstallApps.length > 0){
        ...
        if (device.q['X-Device-Platform'].toUpperCase() === 'W16T') {
          if (device.q['X-Device-Language'].toUpperCase() === 'EN-GB') {
            ...
            tmpStr = 'Bu monitorde yuklu de?il';
          }
        }
  ...
  var onMenuLoaded = function(e, response) {
    ...
    // FC 마이페이지 menu list '결제조회' 제거
    if ($scope.listData.menuList) {
      for (var i = 0 ; i < $scope.listData.menuList.length ; i++) {
        if ($scope.listData.menuList[i].code === '3'){
          $scope.listData.menuList.splice(i, 1);
        }
      }
    }
  ...
  var moveFocusToItem = function(className, item, keyCode) {
    ...
    if (item === 'drawer') {
      ...
      for (var i = 0 ; i < keys.length ; i++) {
        ...
        // [QEVENTSIXT-20375] 상단 메뉴에서 아래 App List 영역으로 포커스 이동 안됨
        var useDefault = false;
        if (!lastItemFocus || !lastItemFocus.item) {
          useDefault = true;
        } else {
          var tokens = lastItemFocus.item.replace('item', '').split('-');
          if (tokens.length === 2) {
            // 실제 element가 있는지 확인
            var row = parseInt(tokens[0], 10);
            var col = parseInt(tokens[1], 10);
        ...
        if (!lastItemFocus.item ||
          ($element[0].getElementsByClassName(lastItemFocus.item).length < 1)) {
          // 그래도 item이 없으면, 메뉴로 이동
          $scope.focusToMenu({x: 0, y: 0, width: 0, height: 0});
          return true;
        }
  ...
  var moveFocusFromMap = function(currentItem, keyCode) {
    ...
    var focusMap = [
    {
      ...
      'to': {
        'left': {'valid': isListEmpty(), 'item': 'menu'},
        'right': {'class': 'btn-update-all', 'item': 'updateall'},
        'up': {'item': 'drawer'},
        'down': {
    ...
    }, {//[QEVENTSIXT-2962] mypage메뉴리스트에서 item 없을 때 가지않도록 처리함
      'from': '1',
      'to': {
        'up': {
        ...
        'right': {'valid': isListEmpty(), 'item': 'menu'}
      }
    }, {
      'from': '2',
      'to': {
  ...
  $scope.focusToHeader = function(rect, keyCode) {
    ...
    if($scope.selectedMenu === "2") {
      ...
      if ($scope.isUpdateAvailable && !$scope.hideListContent && !$scope.isUpdating()) {    // disabled 상태와 똑같은 조건이어야 함
        ...
        //[QEVENTSIXT-2962] item에서 up 버튼 시 drawer 영역으로 가도록 함
        //[QEVENTSIXT-4386] mypage에서 focus가 scroll prev상태 ->up키 입력시 drawer의 mypage로 이동하게 함.
        if(keyCode === keyHandler.UP || focusManager.getCurrent().target === 'prev') {
          moveFocusToItem('', 'drawer', keyHandler.UP);
        } else {//drawer영역이 스크롤에서 부터 이동 되었으면 down키 다시 scroll로 이동
          if (lastItemFocus.isFrom === false){
  ...
  var appUpdateAct = function() {
    ...
    for (i = 0 ; i < apps.length ; i++) {
      ...
      //[QEVENTSIXT-5774] $scope.listData.itemList의 bPremium,purchased response값이 빠져 처리 부분 주석 처리함
      //update all 버튼 누를 시 비로그인 시 멤버쉽 페이지로 이동하는 현상 수정
...
//      var tmpStr = msgLang.apps_install_17_1 + ' ' + msgLang.apps_install_17_2;
  ...
  var appStatusChangeUpdate = function(e, response) {
    try {
      ...
      switch (response.statusValue) {
        ...
        case 7:
          if (response.id) {
            setProgressVisible(true, response.id, 1);
          }
          break;

        /* DOWNLOADING (다운로드 중 프로그래스바 변경) */
        case 8:
          if (response.details.progress && response.details.progress !== 0) {
            setProgressVisible(true, response.id, response.details.progress);
          }
          break;
        ...
        case 26:
        case 31: // 삭제완료
          setProgressVisible(false, response.id);
          console.log('[' + new Date().getTime() + '], ' + response.id + ' : DOWN COMPLETE~~');
          break;
        ...
        case 35:
        case 36:
        case 37: // app parsing 81~83
        case 32:
        case 33:
        case 34: // app closing 84~86
        case 11:
        case 12:
        case 13: // install ready 87~89
        case 17: // install 90
        case 27:
        case 28:
        case 29: // install 91~93
        case 256:
        case 257:
        case 258: // install RO 94~96
        case 265:
        case 266:
        case 267: // post INSTALL 97~99
          setProgressVisible(false, response.id);
          break;
      }
    } catch(e) {
  ...
  var setProgressVisible = function(showHideType, appId, progressPercent) {
    if(!appId) return;
    if(progressPercent && document.getElementsByClassName('progress-apps')) {
      if(document.getElementById(appId)) {
        document.getElementById(appId).style.width = progressPercent + '%';
      }
      if(document.getElementById(appId) && document.getElementById(appId).parentNode && document.getElementById(appId).parentNode.classList.contains('progress-apps')) {
        document.getElementById(appId).parentNode.style.display = 'block';
      }
    } else {
      if(document.getElementById(appId) && document.getElementById(appId).parentNode && document.getElementById(appId).parentNode.classList.contains('progress-apps')) {
        document.getElementById(appId).style.width = '0%';
        document.getElementById(appId).parentNode.style.display = 'none';
      }
    }
  };
  ...
  var onAppUpdated = function(e, appId) {
    ...
    for (var i = 0 ; i < lastDataReceived.jsonObj.appUpdateList.appUpdateCheck.length ; i++) {
      if (lastDataReceived.jsonObj.appUpdateList.appUpdateCheck[i].id === appId) {
        ...
        //[QEVENTSIXT-7409]  Update 완료 후 설치된 앱의 시간을 Update해 줌
        //업데이트 된 앱 부터 0 index의  data 순서를 위치 시킴
        lastDataReceived.jsonObj.appUpdateList.appUpdateCheck[i].installedDate = util.getCurrnetDate();
        var updateObj = lastDataReceived.jsonObj.appUpdateList.appUpdateCheck[i];
        lastDataReceived.jsonObj.appUpdateList.appUpdateCheck.splice(i,1);
        lastDataReceived.jsonObj.appUpdateList.appUpdateCheck.unshift(updateObj);
        break;
      }
    }
  ...
  $scope.initialize = function() {
    ...
    //mypage update 시 앱 설치 날짜를 갱신하기 위해 dateformat api의 response data를 변수에 설정
    if (!device.isLocalJSON && device.updatedateformat === '') {
