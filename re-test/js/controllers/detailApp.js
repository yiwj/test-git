...
app.controller('appDetailController', function($scope, $controller, $element, $rootScope, $timeout, server, focusManager, keyHandler, marquee, util, device, app, getAppInfo, getAppPurchase, appLaunch, getInstallCapacity, appInstall, appInstallCancel, nsuUpdateStatus, membership, toast, appUpdateCheck, storageCapacity, usbList, storage, appService, billing, getPackageInstallCapacity, packageAppInstall, pmLog, eventKey, timeOutValue, adultProcess, spaceInfo, userID, appClose, audioGuidance, util, adManager, nsuGetCurStatus, restartTV) {
  ...
  var stubUpdateCheck = false;
  var launchStatus = false;
  // youtube WOL관련
  var youtubeWOL = {
    ko : 'YouTube 앱을 설치하면 \'Wi-Fi로 TV 켜기\' 기능이 활성화되어 다른기기로 TV 전원을 켤수있습니다.',
    en : 'Downloading the YouTube app will activate the Turn On via Wi-Fi feature on your TV automatically to enable turning your TV on from other devices.',
    ja : 'YouTubeアプリをインストールすると、「Wi-FiでTVオン」機能が有効になり、他の機器からもTVの電源をオンにすることができます。',
    default : 'Downloading the YouTube app will activate the Turn On via Wi-Fi feature on your TV automatically to enable turning your TV on from other devices.'
  };
  ...
  $scope.rateTxt = msgLang._3d_rating;
  //TODO 번역이 완료되면 수정해야함.
  ...
  $scope.languageCode = device.q['X-Device-Language'].split('-')[0];
  ...
  $scope.isFirstToMoreBtn = false;               //첫 진입 시 실행버튼에서 more버튼 이동 가능 여부
  ...
  $scope.isAdDisplayFlag = false;               //앱상세 화면에 진입했을 시 광고가 1번만 나와야하므로 광고 나왔는지 확인하는 변수
  $scope.spaceAvailableSizeTxt = msgLang.detail_2017_storage_2;

  $scope.spaceAvailableSizeMBTxt = 'MB';
  $scope.ratingAudio = msgLang.audio_2017_rating;

  $scope.iconURL ='';
  $scope.iconColor ='';

  $scope.detailappDeepLink = false; //[NCVTDEFECT-3182] deeplink로 들어온지 확인하는 변수
  ...
  $scope.setFocusItem = function(item, element) {
    //[QEVENTSIXT-13565]처음 상세화면 사방향키 진입을 제외한 실행버튼에서 more버튼으로 포커스 이동 가능 상태 False로 설정
    if ($scope.isFirstToMoreBtn) {
      $scope.isFirstToMoreBtn = false;
    }
  ...
  $scope.audioGuidance = function (scope, target, element) {
    //[QEVENTSIXT-4986] 화면은 Sflash 이미지 나오고 Membership으로 전환되는 과정이나 install button을 발화함.
    //blur일때는 발화안하게 막음
    if(device.isBlur){
      return;
    }
    ...
    if (element && (element.innerText === msgLang.more || element.innerText === msgLang.less)) {
      ...
      //등급
      if (element.parentElement.parentElement.querySelector(".synopsis-ageTypeTxt") && element.parentElement.parentElement.querySelector(".synopsis-ageTypeTxt").innerText.length > 0 && device.countryCode !='BR') {
        enterSound += '. ';
        enterSound += msgLang.rating +' ('+element.parentElement.parentElement.querySelector(".synopsis-ageTypeTxt").innerText+')';
      }
      //seller
      ...
      //updateDate
      if (element.parentElement && element.parentElement.querySelector("[ng-if='detailAppData.updateDate']") && element.parentElement.querySelector("[ng-if='detailAppData.updateDate']").innerText.length > 0) {
        params.text += '. ';
        params.text += element.parentElement.querySelector("[ng-if='detailAppData.updateDate']").innerText;
      }
      //Version
      if (element.parentElement && element.parentElement.querySelector("[ng-if='detailAppData.version']") && element.parentElement.querySelector("[ng-if='detailAppData.version']").innerText.length > 0) {
        params.text += '. ';
        params.text += element.parentElement.querySelector("[ng-if='detailAppData.version']").innerText;
      }
    } else if (target === 'btnState') {
      params.text = element;
      isRunningApp = true;
    }else if(target === 'rating'){
      //params.text ="rate this. button'+"%n+' star rating';
      //[QEVENTSIXT-5927] 평가하기 버튼 발화 방법 변경
      params.text += msgLang.rateThis;

      if (getStarsValue(element, 'on')) {
        starValue = getStarsValue(element, 'on');
        params.text += '. ';
//        params.text += starValue;
        params.text += msgLang.audio_button_button;
        params.text += '. ';
        params.text += $scope.ratingAudio.replace('[SCORE]', starValue);
      }
//      params.text += ' '+ $scope.ratingAudio;
    }
    ...
    if ($rootScope.isNewPage) {
      if (scope.getScopeElement()[0]) {
        ...
        if (enterSound.length > 0) {
          ...
          //update Date
          if (element.parentElement.parentElement.querySelector("[ng-if='detailAppData.updateDate']") && element.parentElement.parentElement.querySelector("[ng-if='detailAppData.updateDate']").innerText.length > 0) {
            enterSound += '. ';
            enterSound += element.parentElement.parentElement.querySelector("[ng-if='detailAppData.updateDate']").innerText;
          }
          //Version
          if (element.parentElement.parentElement.querySelector("[ng-if='detailAppData.version']") && element.parentElement.parentElement.querySelector("[ng-if='detailAppData.version']").innerText.length > 0) {
            enterSound += '. ';
            enterSound += element.parentElement.parentElement.querySelector("[ng-if='detailAppData.version']").innerText;
          }
        }
...
//    focusManager.setCurrent($scope, '');  //[WOSLQEVENT-59194] 앱 상세에서 앱 리스트로 이동 시(back키) 포커스 상실
  ...
  var doRateAction = function () {
    ...
    if (dataClickAct !== 'launch' && dataClickAct !== 'update') {
      didInstall = false;
    }
    // 구매 후 평가 가능
    if (!didPurchase) {
      ...
      if (focusObject.scope == $scope) {
        ...
        if (target == 'back') {
          ...
          if (target.indexOf('recomm') >= 0){
            ...
            toggleApp(index);
            //[QEVENTSIXT-32396] more,less 버튼클릭시 발화안함 처리 detailList와 동일하게 처리 함.
            ...
            var scrollY;
            var hidden = false;
            if($element[0].querySelector('.focus')) {
              var tempEl =$element[0].querySelector('.focus');
  ...
  var drawDetail = function(e, response) {
    ...
    try {
      ...
      if (device.fromDeepLink) {
        ...
        $scope.detailappDeepLink = true;
      } else {
        $scope.isDeepLink = false;
        $scope.detailappDeepLink = false;
      }
      ...
      //[QEVENTSIXT-5786] search에서 앱 상세 들어올때 전시되어있지 않은 앱일 경우 앱 Title, 상세 정보 사라짐
      ...
      if ($scope.$id != response.scopeId) { // scope id를 비교하여 중복 호출을 방지함.
        ...
        return;
      }else if(!response.appDetail.error) {
        //[QEVENTSIXT-10138] 에러가 아닐때 해당 값을 설정해줌.
          $scope.iconURL = $scope.detailAppData.iconURL;
          $scope.iconColor = $scope.detailAppData.iconColor;
          $scope.detailAppData.iconURL = '';
      ...
      if (response.appDetail.error) {
        ...
        $rootScope.pageManager.movePageError(errorCode, $scope, $element, device.param.hotkey);
        return;
...
//        }
        // youtube WOL 대응
        if ($scope.detailAppData.id.toLowerCase() === 'youtube.leanback.v4') {
          if (device.q['X-Device-Language'].indexOf('en') !== -1) {
            if (device.useSettopBox) {
              youtubeWOL.en = youtubeWOL.en.replace(/TV/gi, 'UHD Broadcast Box');
            }
            $scope.detailAppData.description = $scope.detailAppData.description + '\n' + youtubeWOL.en;
          } else if (device.q['X-Device-Language'].indexOf('ko') !== -1) {
            if (device.useSettopBox) {
              youtubeWOL.ko = youtubeWOL.ko.replace(/TV/gi, '지상파 UHD 수신 박스');
            }
            $scope.detailAppData.description = $scope.detailAppData.description + '\n' + youtubeWOL.ko;
          } else if (device.q['X-Device-Language'].indexOf('ja') !== -1) {
            $scope.detailAppData.description = $scope.detailAppData.description + '\n' + youtubeWOL.ja;
          } else {
            $scope.detailAppData.description = $scope.detailAppData.description + '\n' + youtubeWOL.default;
          }
        ...
        $scope.useCapabilityTxt;

        var useCapabilityTmpTxt ='';
        if($scope.useCapability && $scope.useSingleApp){
          if($scope.useGamePad){
            useCapabilityTmpTxt += $scope.capabilityGamePadTxt;
          }
          if($scope.useInternet){
            if(useCapabilityTmpTxt.length>0){
              useCapabilityTmpTxt += ', '
            }
            useCapabilityTmpTxt += $scope.capabilityInternetTxt;
          }
          if($scope.use3d){
            if(useCapabilityTmpTxt.length>0){
              useCapabilityTmpTxt += ', '
            }
            useCapabilityTmpTxt += $scope.capability3dTxt;
          }
          if($scope.useCamera){
            if(useCapabilityTmpTxt.length>0){
              useCapabilityTmpTxt += ', '
            }
            useCapabilityTmpTxt += $scope.capabilityCameraTxt;
          }
          if($scope.useMagic){
            if(useCapabilityTmpTxt.length>0){
              useCapabilityTmpTxt += ', '
            }
            useCapabilityTmpTxt += $scope.capabilityMagicTxt;
          }
          $scope.useCapabilityTxt = useCapabilityTmpTxt;
        }

        $scope.detailAppData.ageTypeTxt = msgLang.apps_category01;
        $rootScope.pageManager.setTitle($scope.detailAppData.name);

        /* Spec out 이벤트 날짜 정보 설정 */
//        $scope.detailAppData.eventDate = $scope.detailAppData.eventStartDate && $scope.detailAppData.eventEndDate ? true : false;
        ...
        if ($scope.detailAppData.event == 'Y') {
          ...
          $scope.detailAppData.dpPrice1 = msgLang._3d_price_free;
          if($scope.detailAppData.bItem){
            $scope.detailAppData.dpPrice1 = msgLang.thumb_2017_apps_1 || 'Free (In-app purchase)';
          }
        }
        ...
        /* Spec out */
        /* bPremium 변수값 Spec out으로 인한 강제 입력 */
        $scope.detailAppData.bPremium = false;
        ...
        if (!$scope.usePackageApp) {
          ...
          getInstallCapacity.getRequiredAppSize(requestParam, function(respon) {
            if(respon) {
              ...
              if(packTotalSize){
                packTotalSize = parseInt(packTotalSize) + 102400;
              }
              convertSize(parseInt(packTotalSize?packTotalSize:0) + parseInt(unPackTotalSize?unPackTotalSize:0));
            }
            util.async(function() {
              $scope.drawed = true;
              initializeScroll();
              restoreScrollPos();
              $scope.$emit('finishDraw', $scope.scopeName, timeOutValue.FINISH_DRAW, $scope.detailAppData.id);
            });
          });
        } else {
          util.async(function() {
      ...
      if($scope.isDeeplink){
        errorParam = {
          ...
          popupDesc: msgLang.alert_2017_nopage_1,
          popupBut1: msgLang.ok,
          popupButAct1 : 'closeAppPopup'
        };
    ...
    try {
      ...
      if ($scope.usePackageApp) {
        ...
        *  state : 코드값을 세분화환 내용(device팀에서 내부적으로 쓰기 위해서 만든 값) */
        ...
        if (currentAppStatus) {
          console.log('#### currentAppStatus : ' + JSON.stringify(currentAppStatus));
          if (currentAppStatus.details.paused === true) {
            ...
            if(util.isAWSServer()){
              if(currentAppStatus.details.state === 'download failed') {
                ...
                //[QEVENTSIXT-13004],[QEVENTSIXT-13005] Update 문구 써있지않음
                //앱업데이트 실패 후 installAndUpdateCheckFromOthers 이벤트를 탐.(이전 param을 저장해놓음)
                requestParam = {id: $scope.detailAppData.id, callbackEvent: 'installAndUpdateChecked'};
                getAppInfo.call(requestParam);
            ...
            if (currentAppStatus.statusValue === 30) {
            ...
            }else if (currentAppStatus.statusValue === 24) {
              //[QEVENTSIXT-10452] 대용량 앱 두 개 연속 설치 시 하나의 앱이 download 완료 후 installing 될 때 용량 부족으로 설치가 안됨.
              // currentAppStatus.statusValue 24에 대하여 처리 함.
              $scope.progressPercent = 0;
              setProgressVisible(false);
              ...
              setActionButton('installing');
              installProcessDelegator(currentAppStatus);
              pmLog.write(pmLog.LOGKEY.STUB_APP, { key : 'initializeDetail- ( 7 )' });
              //[QEVENTSIXT-10451] 해당 로직타면 포커스가 안생김.
              //설치중 버튼이 disabled이고 마우스가 아니면 포커스가게 수정
              //월래 setActionButton 함수에서 타야하지만 !$scope.enableButton에 대한 로직이 없어서 그러함.
              if(!$scope.enableButton && !document.body.classList.contains('hover-mode') && !document.body.classList.contains('focus')){
                $scope.setDefaultFocus();
              }
            }
          } else {
  ...
  var hideList = function(e, page) {
    ...
    try {
      ...
      //[NCVTDEFECT-3888] 홈런처- '채널+' 앱 클릭時 업데이트 화면과 LG 스토어 메인화면 겹침 및 클릭무감
      //가끔씩 page == '' 조건을 탐. (방어로직)
      var documentPages = document.getElementsByClassName('page');
      if(documentPages && documentPages.length >= 2){
        page = 'detailApp';
      }
      ...
      if ((page != $scope.scopeName || $scope.isInLink) && !$scope.showing) {
        if (($scope.direct == false && $scope.showing == false)) {
          if (page == '') {
            ...
            //[QEVENTSIXT-10254] 썸네일 출력하지 않음(새로고침하면 앱 아이콘 및 배경화면이 안나옴.)
            $scope.detailAppData.iconURL = $scope.iconURL;
            $scope.detailAppData.iconColor = $scope.iconColor;
            /*  넷플릭스일때만 상세에서 컬러 지운다.  */
            ...
            //앱 배경화면이 존재하지 않을 경우 앱 배경화면 지정
            if(!$scope.detailAppData.iconColor) {
              $scope.detailAppData.iconColor = '#3c3e41';
            }
            ...
            $scope.direct = false;
            $element[0].classList.remove('direct');
...
//        }
  ...
  $scope.moveFocusByKey = function(keyCode) {
    ...
    if ($scope.focusItem == '') {
      if (focusManager.getLastFocus().scope.scopeName !== "drawer" && focusManager.getLastFocus().scope.scopeName !== "depth3Ad") {
        device.isFocusItem = false;
      }

      if (lastItemFocus && !lastItemFocus.isFrom && Object.keys(lastItemFocus).length > 0 && focusManager.getCurrent().target != 'depth3Ad') {
        // The last focus was scroll btn.
        rect = {x: 0, y: 250, width: 0, height: 100};
        $scope.$parent.$broadcast('focus', 'scroll', keyHandler.RIGHT, rect);
        return false;
      }
      if (focusManager.getLastFocus().scope.scopeName === 'drawer' && focusManager.getPrevious().target === 'depth3Ad' &&
        $scope.isAdDisplayFlag && $rootScope.depth3Ad.showVideoPlayer) {
        $rootScope.$broadcast('focus', 'depth3Ad', function () {
          // right button이 섵택되었을 때 실행될 callback
          var currItem = 'depth3Ad';
          var currElement = document.querySelector('[item="' + currItem + '"]');
          $scope.setFocusItem(currItem, currElement);
        });
        return;
      }else if(focusManager.getLastFocus().scope.scopeName === 'depth3Ad' && keyCode === keyHandler.RIGHT) {
        //17년도 광고에서 포커스 있고 오른쪽키(사방향) 눌렀을 때 스크린샷으로 가야함.
        moveFocusFromInstall(keyCode);
        return;
      }else if(focusManager.getLastFocus().scope.scopeName === 'depth3Ad' && keyCode === keyHandler.DOWN) {
        $scope.setDefaultFocus();
        return;
      }else{
        $scope.setFocusItem(lastFocus.item, lastFocus.element);
    ...
    switch (keyCode) {
      ...
      case keyHandler.UP:
        if (row == 0) { // 첫번째 row이면 header로 이동
          //[QEVENTSIXT-27421] screenshots에서 평가하기가 disabled되어있어 포커스가 안감.
          if ($scope.useRating && useRatingBtn && !stubUpdateCheck) {
        ...
        if ($scope.row[row-1].type !== $scope.row[row].type && $scope.row[row-1].type != 'button') {
          //17년향 같은 row이면
          if($scope.row[row].type === 'button') {
            if($scope.row[row-1].count%2 === 1){
              index = $scope.row[row - 1].index;
            }else{
              index = $scope.row[row - 1].index +1;
            }
          }else{
            //미리보기
            if($scope.row[row - 1].count >=2){
              if(index >= 1) {
                index = $scope.row[row - 1].index + 1;
              }else{
                index = $scope.row[row - 1].index;
      ...
      case keyHandler.RIGHT:
        //[QEVENTSIXT-7896] 우측에 scroll ui 출력 안되는 app -> screen shots 항목에 포커스 있는 상태에서 포커스 제일 우측으로 이동 시
        //포커스 사라짐 (audio guidance 음성으로 scroll down이라고 발화함) - 재현이 안되므로 방어로직 넣음.
        if(remainCnt == 0 && $element[0].querySelector('[item="next"]').classList.contains('disabled')){
          //[QEVENTSIXT-14798] 하단 scroll disabled이고 상단 scroll만 보일때 상단으로 포커스 갈 수 있도록 수정
          if(remainCnt == 0 && $element[0].querySelector('[item="prev"]').classList.contains('disabled')){
            return;
          }
        }
        //[QEVENTSIXT-13564]  포커스 사라지고, "Scroll down"이라고 발화됨
        if(remainCnt == 0 &&  $element[0].querySelector('[item="next"]').style.display === 'none') {
          return;
        }
        if (remainCnt == 0) {
          rect = {
      ...
      case keyHandler.DOWN:
        ...
        if ($scope.row[row+1].type !== $scope.row[row].type) {
          ...
          if (index<0) {
            index = $scope.row[row+1].itemPerRow - 1;
...
//            index = $scope.row[row + 1].index;
//          } else {
//            if($scope.row[row+1].prefix !== $scope.row[row].prefix){
//              index = $scope.row[row + 1].index;
//            }else{
//              index = ($scope.row[row + 1].count + $scope.row[row + 1].index) - 1;
//            }
//          }
        ...
        name = 'item-' + $scope.row[row + 1].prefix + '-' + index;
        element = $element[0].getElementsByClassName(name)[0];
        //[QEVENTSIXT-14799] Screenshots 항목 3개 / Seller Recommends 항목 1개 존재한 경우 2,3항목에서
        //아래 키 누르면 아래로 포커스 이동 안함.
        if (!element) {
          for (var i = 0; i < index; i++) {
            //element 없을때에는 그 옆에꺼 선택
            name = 'item-' + $scope.row[row + 1].prefix + '-' + (index-1);
            element = $element[0].getElementsByClassName(name)[0];
            if(element){
              //화면에 값이 존재하지 않을때는 한번 더 체크함.
              i = index;
            }
          }
        }
...
//        if ($scope.row[row + 1].type == 'button' && $scope.row[row + 2]) {  // next focus item 이 버튼이면 하위 아이템으로 스크롤되게 한다.
//          tempEl = $element[0].getElementsByClassName('item-' + $scope.row[row + 2].prefix + '-0')[0];
//          if (tempEl) {
//            scrollY = tempEl.getBoundingClientRect().top + tempEl.offsetHeight - scroll.wrapperOffsetTop - scroll.y;
//          }
//        }
        //[QEVENTSIXT-7416] 맨 아래 item 이동시 scroll이 맨 밑 까지 내려가지 않는 이슈 처리.
        if(row+2 === $scope.row.length) {
          scrollY += 130;
        }
        if (scrollY > scroll.wrapperH - scroll.y) {
          hidden = true;
  ...
  var moveFocusFromRating = function(keyCode) {
    switch (keyCode) {
      case keyHandler.UP:
        ...
        break;
      case keyHandler.DOWN:
        if($scope.isAdDisplayFlag && $rootScope.depth3Ad.showVideoPlayer) {
          $rootScope.$broadcast('focus', 'depth3Ad', function() {
            // right button이 섵택되었을 때 실행될 callback
            var currItem = 'depth3Ad';
            var currElement = document.querySelector('[item="'+currItem+'"]');
          ...
          if ($element[0].getElementsByClassName('btn-large')[0]) {
            $scope.setFocusItem('button', $element[0].getElementsByClassName('btn-large')[0]);
          }
        }
        break;
    }
    ...
    switch (keyCode) {
      ...
      case keyHandler.UP:
        if($scope.isAdDisplayFlag && $rootScope.depth3Ad.showVideoPlayer) {
          ...
          $rootScope.$broadcast('focus', 'depth3Ad', keyCode, {x: 2000, y: 100, width: 0, height: 0});
          //버튼에서 광고가 재생되는 경우
          break;
        }
        if ($scope.useRating && useRatingBtn && !stubUpdateCheck) {
        ...
        break;
      case keyHandler.RIGHT:
        //[QEVENTSIXT-13565]처음 상세화면 사방향키 진입 시 실행버튼에서 more버튼으로 포커스 이동
        if($scope.isFirstToMoreBtn && $element[0].querySelector('[item="prev"]').classList.contains('disabled')) {
  ...
  var getNearContentsElement = function(target, install, installRight) {
    ...
    for (var i = 0; i < $scope.row.length; i++) {
      ...
      if (!install) {
        ...
        if(obj.type == 'button') {
          name = 'item-' + obj.prefix + '-0';
        }
      } else {
        if (obj.type == 'button') continue;
        //back 버튼에서 돌아올 경우 이전에 포커스된 아이템 컨텐츠로 이동한다.
        if (lastItemFocus.item) {
          //[QEVENTSIXT-20007],[QEVENTSIXT-20507] 영역간 이동은 이전 3-1,3-2영역간한단계이전라스트포커스는기억함
          if (!lastItemFocus.isFrom && focusManager.getCurrent().target !== 'prev' && focusManager.getCurrent().target !== 'next' && focusManager.getCurrent().target != 'button'
            && focusManager.getCurrent().target !== 'depth3Ad') {
            // The last focus was scroll btn.
            rect = {x: 0, y: 250, width: 0, height: 100};
            $scope.$parent.$broadcast('focus', 'scroll', keyHandler.RIGHT, rect);
            return false;
  ...
  var focusHandler = function(e, target, keyCode, rect) {
    ...
    var element;
    element = $element[0].getElementsByClassName('btn-small')[0];
    if (element && useRatingBtn && !stubUpdateCheck && (focusManager.getCurrent().scope.scopeName === 'drawer' ||
      (focusManager.getCurrent().scope.scopeName =='depth3Ad' && keyCode === keyHandler.UP))) {
      focusToHeader(rect);
    } else {
  ...
  var focusDefaultDetailApp = function() {
    if(focusManager.getCurrent().target === 'depth3Ad'){
      //포커스가 광고에 존재하고 광고가 없어지면 포커스가 강제적으로 버튼에 가게 수정
      focusToInstall();
      return;
    }
  };
  ...
  var toggleApp = function(index) {
    ...
    if (hidden) {
      scrollByKey = true;
      if($element[0].querySelector('.focus').getAttribute('item') === 'item-recommBtn1-0') {
        if (device.isHD) {
          scroll.scrollTo(0, scrollY - 25, 300, false);
        } else {
          scroll.scrollTo(0, scrollY - 34, 300, false);
        }
      }
      else{
        scroll.scrollTo(0, scrollY, 300, false);
    ...
    $scope.scrollRefresh();
//    scroll.scrollTo(0, 12, 300, false);
    // recommend app list event regist.
    appArr = $element[0].getElementsByClassName('item-apps');
    m = appArr.length;
    for (i = 0; i < m; i++) {
...
//        $scope.isLogin = true;
//      }
      //[QEVENTSIXT-13234],[QEVENTSIXT-20131] 앱설치 후 버튼이 launch로 변경될때 spinner가 돌음-방어로직
...
//          }
        ...
        case 268:
          ...
          $timeout(function() {
            //[QEVENTSIXT-13594],[QEVENTSIXT-17320] App 설치 중 "Installing" 발화 안됨
            //installing과 포커스 이동하면서 screenshots가 거의 동시에 발화되어 installing이 들리지 않는 이슈(시간조정함)
            if(!launchStatus) {
...
//        }
        ...
        /* INSTALL DONE (설치 완료) */
        case 30:
          pmLog.write(pmLog.LOGKEY.ST_APP_DOWN_COMPLETE, { key : 'APPID : '+$scope.detailAppData.id });
          ...
          //불필요한 API호출 제거(refreshScreen에서 갱신됨.)
//          requestDetailDataSmall(); //구매정보가 갱신되어야 하므로 호출
          /* Stub App 자동 업데이트 시 앱설치가 완료가 되면 버튼을 UPDATE로 세팅 */
          if (appStatusInfo.details && appStatusInfo.details.type === 'stub') {
            //FC 대응 - 강제 업데이트 후에 update버튼 클릭해도 키무감
            //$scope.updateCheckType 설정이 안되어있어 해당 값 재설정
            getAppInfo.call(requestParam);
            setActionButtonEnable(true);
//            setActionButton('update');
        ...
        case 23:
          //[QEVENTSIXT-24393] 앱 설치 실패하고 spinner뜸
          $scope.spinner = false;
          removeSpinner();
          setProgressVisible(false);
          getAppInfo.call(requestParam);
  ...
  var installAndUpdateCheck = function(e, response) {
    ...
    try {
      ...
      if( checkResult.returnValue === true && checkResult.appInfo && $scope.detailAppData) {
        ...
        //[QEVENTSIXT-13581] Stub앱 시 Rating 불가 처리 및 4방향키 포커스 상실 버그 처리
        if ($scope.detailAppData.isStub) {
          stubUpdateCheck = true;
          useRatingBtn = false;
        ...
        if(!$scope.detailAppData.appInfo.binId){
          $scope.detailAppData.binId = 0;
        }else {
          $scope.detailAppData.binId = $scope.detailAppData.appInfo.binId;
        }
        ...
        // 앱 상세페이지 에서 평가하기 버튼에 별점 정보 노출
        if($scope.detailAppData.evaluationPoint){
          var ratingRatio = 10 * parseInt($scope.detailAppData.evaluationPoint, 10);
          for (var i = 0; i < 5; i++) {
            if (i <= ((ratingRatio / 20) - 1)) {
        ...
        /*  업데이트 체크 */
        requestCheckUpdate($scope.detailAppData.id, $scope.detailAppData.binId, checkResult.appInfo.version);
      } else {
        /*설치 안되어 있음
         미설치, 인스톨 가능 여부 판단.*/
        //[WOSLQEVENT-109614] 이상희 연구원 요청으로 평가하기 버튼 앱 미설치시 disabled 처리
        ...
        if (appInstallData.error.code == 'A.001.02') {
          ...
          if (device.currentPage === 'detailApp') {
            ...
            requestParam = {
              type: 'error',
              popupTitle: msgLang.alert_adult_3_2,
              errorMsg: msgLang.alert_2017_network_2, //[QEVENTSIXT-19200]2017년향 문구에 맞게 변경
              errorCodeMsg: 'ERROR CODE: '+errorCode
            };
            $rootScope.popupApp.showPopup($scope, requestParam);
            $scope.spinner = false;
        ...
        if (appUpdateInfo.appCount == 0) {
          ...
          if (appUpdateInfo.appUpdateCheck && appUpdateInfo.appUpdateCheck.length > 0) {
            if (appUpdateInfo.appUpdateCheck[0].errorCode) {
              ...
              //2017년향 응답값 bPurchased 값 없어짐.(영향도 체크 필요함.)
              if(appUpdateInfo.appUpdateCheck[0].bPurchased){
...
//      }
      ...
      //[17년도 변경내역] launch의 경우에만 서버에서 받아온 용량을 표시
      if(dataClickAct == 'launch') {
        appUpdateSize('launch');
      }
    } catch (e) {
      //[WOSLQEVENT-75212] 가끔씩 일시적 장애 팝업이 출력되며 무한로딩 발생
      removeSpinner();
  ...
  var installStart = function(installTarget) {
    //[QEVENTSIXT-11501] 성인앱 설치전에 팝업이 떠 있는지 확인(방어로직)
    if($rootScope.popupApp && $rootScope.popupApp.open &&
      ($rootScope.popupApp.desc === msgLang.alert_2017_adult_5 || $rootScope.popupApp.desc === msgLang.alert_adult_2_6)){
      return;
    }
    var requestParam, packTotalSize, unPackTotalSize, membersParam = [];
...
//      return false; // 유료앱
//    }
    //[QEVENTSIXT-12773] 17년도향은 무조건 무료앱
    return true;
  };
  ...
  var genErrorCode = function(errorCodeVal) {
    ...
    if( errorCodeVal === 'ERR.401' ) {
      errorCode = '401';
    } else if( errorCodeVal === 'noData' ) {
      errorCode = '001';
    } else if( errorCodeVal === 'A.014.01' ){
      errorCode = '011';
  ...
  var installCapacityResult = function(e, response) {
    ...
    try {
      ...
      if (capacityResult.returnValue === true) {
        ...
        if(device.auth.adultStatus != 'Verified' && $scope.isAdult){
        //성인앱이고 성인인증이 안된 경우 아래 팝업이 안뜨게함.(로그인 팝업이 떠야함)
        }else if ($scope.isAdult && !$scope.fromPopUp && !device.isAdultSettingPopup && device.auth.adultStatus === 'Verified') {
          //[QEVENTSIXT-8477] 성인앱 클릭시 팝업창이 뜨면서 spinner가 돔.
          if($rootScope.spinner.isShowing()){
            $rootScope.spinner.hideSpinner();
            if(document.getElementsByClassName('spinner')[0] && document.getElementsByClassName('spinner')[0].style){
              document.getElementsByClassName('spinner')[0].style.visibility = 'hidden';
              $rootScope.spinner.hide = true;
            }
          }
          requestParam = {
            type: 'popup',
            ...
            //TODO 2017문구 msgLang.alert_2017_adult_6
            popupDesc: msgLang.alert_2017_adult_6
          };
          $rootScope.popupApp.showPopup($scope, requestParam);
          //[QEVENTSIXT-2677]성인인증 팝업을 한번만 떠야함.
          device.isAdultSettingPopup = true;
          return;
        // 성인앱이 아닌 경우
        } else {
      ...
      /* 설치가 불가능한 경우(용량 부족으로 USB 리스트를 출력)*/
        ...
        if ($scope.detailAppData && $scope.detailAppData.appFile && $scope.detailAppData.appFile.internalInstOnly) {
          // 2016-11-04 ATSC3.0 STB 일경우 문구 변경\r 	3825
          var tmpStr = msgLang.alert_2017_nostorage_1;
          if (device.useSettopBox) {
            if (device.q['X-Device-Language'].substring(0, 2).toUpperCase() === 'EN') {
              tmpStr = tmpStr.replace(/TV/gi, 'UHD Broadcast Box');
            }
          }
          ...
          // 외부 저장소(USB)가 없는 경우(용량X, 외부저장소X) 토스트 메시지 출력
            ...
            toast.call({
              msg:msgLang.alert_2017_nostorage_2,
              persistent:true,
              noaction:false,
...
//        var tmpStr = msgLang.apps_install_17_1 + ' ' + msgLang.apps_install_17_2;
          ...
          if (device.q['X-Device-Platform'].toUpperCase() === 'W16T') {
            if (device.q['X-Device-Language'].toUpperCase() === 'EN-GB') {
              ...
              tmpStr = 'Ba?ka monitorden yuklenen uygulamalar var. USB aygıtını bicimlendirin ve farklı bir monitorde yuklu olan uygulamaları silin.';
            }
          }
  ...
  var softwareUpdateRequiredPopup = function () {
    requestParam = {
      ...
      popupDesc: msgLang.alert_2017_update_4
    };
    $rootScope.popupApp.showPopup($scope, requestParam);
  };
  ...
  var installFromPopup = function() {
    //[QEVENTSIXT-10203] 키 동작하지 않음.
    //msgLang.alert_adult_2_6 팝업이 뜬 후에 마치 spinner가 떠 있는것처럼 생각되어
    //키무감이 발생됨. - 한번더 체크하는 방어로직
    if($rootScope.spinner.isShowing()){
      $rootScope.spinner.hideSpinner();
    }
...
//    nsuUpdateStatus.call();
  ...
  var usbPopupListUpdate = function() {
    ...
    $scope.usbListData = app.usbListData;
    if( $scope.usbListData.length == 0 ) {
      //[QEVENTSIXT-12164] 팝업창 출력된 상태로 대기 시 자동으로 팝업창 닫힘
      //팝업창이 네트워크 팝업창이면 팝업창 닫히지 않게 수정
      if(!$rootScope.popupApp.hide && $rootScope.popupApp.title === msgLang.alert_adult_3_4){
        return;
      }
      if (!$rootScope.popupApp.hide) $rootScope.popupApp.hidePopup();
    } else {
...
//    }
    //[QEVENTSIXT-4392] 17년향 소스에서 평가하기 팝업창 없앰.
...
//            $rootScope.popupApp.showPopup($scope, requestParam);
            document.querySelector('[item="rating"]').classList.remove('disabled');
            //[QEVENTSIXT-10051] 평가하기 버튼에 포커스 넣어줌.
            $scope.setFocusItem('rating', document.querySelector('[item="rating"]'));
          }
          clearInterval(updateDetailTime);
        }else{
          requestDetailData();
...
//        $scope.detailAppData.bPurchased = response.appDetail.app.bPurchased;
  ...
  var updateSpaceInfo = function() {
    ...
    $scope.spaceAvailableSize = parseInt(device.spaceFreeSize.split(' '));
    //[QEVENTSIXT-25961] LG Content Store＞앱 상세페이지 일본어 틀림
    $scope.spaceAvailableSizeTxt = (msgLang.detail_2017_storage_2).replace('[STORAGE]', $scope.spaceAvailableSize+$scope.spaceAvailableSizeMBTxt);
    pmLog.write(pmLog.LOGKEY.SPACE_LOG, {
  ...
  var refreshScreen = function(e, paramObj) {
    ...
    if (paramObj && paramObj.app && $scope.appId !== paramObj.app.id && $scope.detailAppData.id !== paramObj.app.id) {
      //패키지 앱이 아닐경우
    ...
    if (paramObj && paramObj.change) {
      ...
      switch (paramObj.change) {
        ...
        case 'added':
        case 'updated':
          //FC대응 - 패키지앱이 없어졌으므로 로직 변경 - 버튼이 installing에서 launch로 변경안되는
          //이슈에 대한 방어로직
          if (paramObj.app && ($scope.detailAppData.id === paramObj.app.id)) {
            $scope.usbStatus = paramObj;
            launchStatus = true;
            //17년향 변경내역 - 앱 설치 후 앱 용량이 변경됨.(앞으로 update될 용량만 표시됨)
            appUpdateSize('launch');
            ...
            setActionButton('launch');
            removeSpinner();
            //[QEVENTSIXT-13564] USB에 앱 삭제한 경우 평가하기 버튼 비활성화함.
            if(document.querySelector('[item="rating"]').classList.contains('disabled')) {
              document.querySelector('[item="rating"]').classList.remove('disabled');
              $scope.useRating = true;
              useRatingBtn = true;
              if($scope.detailAppData.evaluationPoint){
            ...
            //[QEVENTSIXT-14125] 앱 설치 후 앱 버튼이 변경되지 않는 이슈
            pmLog.write(pmLog.LOGKEY.ST_CHANGE_BTN, {
              response : JSON.stringify(paramObj)
            });
          }
          break;
        case 'removed':
...
//      //[QEVENTSIXT-13601] YouTube 앱에 한에서 나오는 팝업
//      if (paramObj.app && paramObj.change === 'added' && ($scope.detailAppData.id === paramObj.app.id) && paramObj.app.id === 'youtube.leanback.v4') {
//        requestParam = {
//          type: 'popup',
//          popupTitle: 'youtube.leanback.v4',
//          popupBut1: msgLang.ok,
//          popupButAct1 : 'closeAppPopup',
//          //TODO 2017문구
//          popupDesc: 'The installation of the YouTube app has been completed and the Turn On via Wi-Fi feature on your TV has also been automatically activated. This may affect energy consumption and you can deactivate the Turn On via Wi-Fi option in the Settings menu.'
//        };
//        $rootScope.popupApp.showPopup($scope, requestParam);
//      }
    } else {
  ...
  var appUpdateSize = function(buttonName) {
    var packTotalSize = $scope.detailAppData.appFileList[0].packFileSize;
    var unPackTotalSize = $scope.detailAppData.appFileList[0].unpackFileSize;
    var requestParam = {'id':$scope.detailAppData.id, 'size':packTotalSize, 'uncompressedSize':unPackTotalSize};
...
//    getAppInfo.call(requestParam);
    ...
    //[QEVENTSIXT-12450] 성인 인증 하라는 팝업 0.5초간 출력후 사라짐(방어로직)
    if(!$rootScope.popupApp.hide && $rootScope.popupApp.desc === msgLang.alert_2017_adult_1){
      $rootScope.popupApp.hidePopup();
    }
    $timeout(function() {
      requestDetailData();
...
//    }
  ...
  var doInstallActionStart = function () {
    /*  설치  */
...
//        nsuUpdateStatus.call();
      ...
      if (($scope.detailAppData.bPremium === false || $scope.detailAppData.isStub === false) && dataClickAct !== 'launch') {
          ...
          if ((dataClickAct === 'install' || dataClickAct === 'update' || dataClickAct === 'pay') &&
            device.auth.adultStatus !== 'Verified' && $scope.detailAppData.adultYN === 'Y') {
            ...
            //[QEVENTSIXT-11550] 성인인증 팝업과 앱설치 후 팝업이 동시에 뜸.
            //actionAfterGetAdultStatus 함수에 대한 응답값이 늦게 와서 아래 로직이 타서 팝업이 2개가 뜰 경우 발생.
            //순차적으로 실행할 수 있게 수정
            doItActionAfterAdultCheck(dataClickAct);
          }
      } else if(dataClickAct === 'launch'){
        doItActionAfterAdultCheck(dataClickAct);
      }else {
        $rootScope.spinner.hideSpinner();
      }
  ...
  var doItActionAfterAdultCheck = function (buttonMsg) {
    var dataClickAct = buttonMsg;

    if (dataClickAct == 'install') {
      doInstallActionStart();
...
//          nsuUpdateStatus.call();
        ...
        if ($scope.updateCheckType == 'YYN' || $scope.updateCheckType == 'AYN' || $scope.updateCheckType == 'FNN' || $scope.updateCheckType == 'FYN' || $scope.updateCheckType == 'NYN') {
          ...
          requestParam = {
            ...
            //TODO 2017문구 msgLang.alert_2017_update_2 (msgLang.apps_install_4_7 + " " + msgLang.apps_install_4_8)
            popupDesc: msgLang.alert_2017_update_2
          };
          $rootScope.popupApp.showPopup($scope, requestParam);
...
//          nsuUpdateStatus.call();
          nsuGetCurStatus.call();
          //softwareUpdateRequiredPopup();
          //[NCVTDEFECT-3182] deeplink로 들어오면 아래 팝업을 띄워주지 않는다.(이상의 연구원 요청)
        } else if (($scope.updateCheckType == 'YNN' || $scope.updateCheckType == 'ANN') && !$scope.detailappDeepLink) {
          /*  실행 하던지 업데이트 하던지 */
          requestParam = {
            type: 'popup',
            ...
            //TODO 2017문구 msgLang.alert_2017_update_1
            popupDesc: msgLang.alert_2017_update_1
          };
          $rootScope.popupApp.showPopup($scope, requestParam);
          //[NCVTDEFECT-3182] deeplink로 들어오면 아래 팝업을 띄워주지 않는다.(이상의 연구원 요청)
        } else if (($scope.updateCheckType == 'YNN' || $scope.updateCheckType == 'ANN') && $scope.detailappDeepLink) {
      ...
      //[QEVENTSIXT-12393] launch할때 스피너가 돌아서 키무감이됌.
      $rootScope.spinner.hideSpinner(); // remove the spinner
      if ($scope.usePackageApp) {
...
//    console.log('device.param ::::::::::::::::' + JSON.stringify(device.param));
    if(device.param && device.param.page === 'detailApp') {
      $scope.isAdDisplayFlag = true;
      $rootScope.depth3Ad.showAD(adRes);
    }
    $rootScope.spinner.hideSpinner();
  };

  /*
   * webkitHidden event 일때 광고 refresh
   */
  var foregroundHandler = function(e) {
    /*이미 광고가 재생되었으면 광고 refresh 수정*/
      adManager.call('requestContextIndex', '', 'detailApp', undefined, $scope.detailAppData.id);
  };

  /*
   * audioGuidance 평가하기 버튼
   */
  var getStarsValue = function(element, valueType) {
    var tmpClass = '.btn-score.';
    tmpClass = tmpClass.concat(valueType);

    if (element && element.querySelectorAll(tmpClass).length > 0 ) {
      if(Array.prototype.pop.call(element.querySelectorAll(tmpClass)).attributes[1].value) {
        var tmpStarValue = Array.prototype.pop.call(element.querySelectorAll(tmpClass)).attributes[1].value;
        switch (tmpStarValue) {
          case "star0":
            tmpStarValue = "1";
            break;
  ...
  $scope.initialize = function() {
    ...
    //2017년향 광고 이벤트
    $scope.$on(eventKey.DEPTH3_AD_LOADED, displayAD);
    $scope.$on('depth3AdReplay', foregroundHandler);
    $scope.$on('focusDefaultDetailApp', focusDefaultDetailApp);
