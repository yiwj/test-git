...
discoveryUtil.service('util', function(headers, device) {
  ...
  //Mypage 에서 앱 업데이트 목록이있고 업데이트 all를 누를 경우 현재 page에서 앱의 날짜를 갱신시키기 위해 나라 format별로 현재 시간 get
  this.getCurrnetDate = function() {
    var dateTmp, dateformat;

    if (device.updatedateformat === ''){
      dateformat = "DD-MM-YYYY";
    }else{
      dateformat = device.updatedateformat;
    }

    var inUsedDate = new Date().toISOString().slice(0,10).replace(/-/g,"");
    dateTmp = dateformat.replace('YYYY', inUsedDate.substring(0, 4));
    dateTmp = dateTmp.replace('MM', inUsedDate.substring(4, 6));
    dateTmp = dateTmp.replace('DD', inUsedDate.substring(6, 8));
...
//    dateTmp = dateTmp.replace(yyStr, yyStr2);
  ...
  this.findFocusDownItemCursor = function(focusElemet){
    var curObj = null;
    var focusLeft, focusTop;

    focusLeft = device.cursorParams.x;
    focusTop = device.cursorParams.y;
//    console.log("11 findFocusDownItemCursor focusLeft : ",focusLeft,", focusTop",focusTop);

    var gapPrev = 0;
    var gapYTop,gapfocusElemet,gapYMid,total;
    if(focusElemet) {
      for (var i = 0; i < focusElemet.length; i++) {
        if(focusElemet[i].classList.contains('disabled')){
          continue;
        }
        gapfocusElemet = focusElemet[i].getBoundingClientRect();
        gapYTop = Math.abs(gapfocusElemet.top - focusTop);


        if(focusTop <= gapfocusElemet.bottom && gapfocusElemet.top <= focusTop){
          gapYTop = 0;
        }

//        console.log("111 focusElemet[i] left,focusLeft,", focusLeft," ,gapfocusElemet.left," , gapfocusElemet.left ,", right," , gapfocusElemet.right);
        if(focusLeft <= gapfocusElemet.right && gapfocusElemet.left <= focusLeft){
          gapYMid = 0;
        }else if( gapfocusElemet.left > focusLeft && gapfocusElemet.right > focusLeft){
          gapYMid = gapfocusElemet.left - focusLeft;
        }else if(gapfocusElemet.left < focusLeft && gapfocusElemet.right < focusLeft){
          gapYMid = focusLeft - gapfocusElemet.right;
        }else{
          console.log("case look for error");
        }

//        console.log("value : ",focusElemet[i],", focusTop : ",focusTop, "gapYTop : ", gapYTop,", gapYMid : ", gapYMid, ", total : ", gapYTop + gapYMid);

        total = gapYTop + gapYMid;
        if(gapYTop === 0 || gapfocusElemet.top >= focusTop){
          if(gapPrev === 0){
            gapPrev = total;
            curObj = focusElemet[i];
          }else{
            if(gapPrev >= total){
              curObj = focusElemet[i];
              gapPrev = total;
  ...
  this.findFocusUpItemCursor = function(focusElemet){
    var curObj = null;
    var focusLeft, focusTop;
    ...
    var gapPrev = 0;
    var gapYBotton,gapfocusElemet,gapYMid,total;
    if(focusElemet) {
      for (var i = 0; i < focusElemet.length; i++) {
        if(focusElemet[i].classList.contains('disabled')){
          continue;
        ...
        gapfocusElemet = focusElemet[i].getBoundingClientRect();
        gapYBotton = Math.abs(focusTop - gapfocusElemet.bottom);
...
//        console.log("value : ",focusElemet[i],", focusTop : ",focusTop, "gapYTop : ", gapYTop,", gapYMid : ", gapYMid, ", total : ", gapYTop + gapYMid);
        ...
        total = gapYBotton + gapYMid;
        if(gapYBotton === 0 || focusTop >= gapfocusElemet.bottom ){
          if(gapPrev === 0){
            gapPrev = total;
            curObj = focusElemet[i];
          }else{
  ...
  this.findFocusRightItemCursor = function(focusElemet){
    var curObj = null;
    var focusLeft, focusTop;
    ...
    var gapPrev = 0;
    var gapYLeft,gapfocusElemet,gapYMid,total;
    if(focusElemet) {
      for (var i = 0; i < focusElemet.length; i++) {
        if(focusElemet[i].classList.contains('disabled')){
          continue;
        ...
        gapfocusElemet = focusElemet[i].getBoundingClientRect();
        gapYLeft = Math.abs(gapfocusElemet.left - focusLeft);

        //만약 포커스가 X좌표로 item사이에 들어있을 경우 X좌표 gap는 0으로 설정
        if(focusLeft <= gapfocusElemet.right && gapfocusElemet.left <= focusLeft){
          console.log("11111");
          gapYLeft = 0;
        }
        ...
        if(focusTop <= gapfocusElemet.bottom && gapfocusElemet.top <= focusTop){
          gapYMid = 0;
        }else if( gapfocusElemet.bottom > focusTop && gapfocusElemet.top > focusTop){
          gapYMid = gapfocusElemet.bottom - focusTop ;
        }else if(gapfocusElemet.bottom < focusTop && gapfocusElemet.top < focusTop){
          gapYMid = focusTop - gapfocusElemet.top;
        }else{
          console.log("case look for error");
        }
//        console.log("11 findFocusRightItemCursor value : ",focusElemet[i],"gapYLeft : ", gapYLeft,", gapYMid : ",gapYMid,", total : ",gapYLeft + gapYMid," , gapfocusElemet.left : ",gapfocusElemet.left," , focusLeft : ",focusLeft);

        total = gapYLeft + gapYMid;
        if(gapYLeft === 0 || focusLeft <= gapfocusElemet.left ){
          if(gapPrev === 0){
            gapPrev = total;
            curObj = focusElemet[i];
          }else{
    ...
    if(curObj !== null){
      console.log("findFocusRightItemCursor curObj : ",curObj);
      return curObj;
    }
  };

  this.findFocusLeftItemCursor = function(focusElemet){
    var curObj = null;
    var focusLeft, focusTop;
    ...
    if(focusElemet) {
      for (var i = 0; i < focusElemet.length; i++) {
        ...
        gapfocusElemet = focusElemet[i].getBoundingClientRect();
        gapYLeft = Math.abs(gapfocusElemet.right - focusLeft);
...
//        console.log("11 findFocusRightItemCursor value : ",focusElemet[i],"gapYLeft : ", gapYLeft,", gapYMid : ",gapYMid,", total : ",gapYLeft + gapYMid," , gapfocusElemet.left : ",gapfocusElemet.left," , focusLeft : ",focusLeft);
        ...
        total = gapYLeft + gapYMid;
        if(gapYLeft === 0 || focusLeft >= gapfocusElemet.right ){
          if(gapPrev === 0){
            gapPrev = total;
            curObj = focusElemet[i];
          }else{
